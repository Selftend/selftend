 ...
 * its constraints).
 *
 * **The `be` ramp, not the design's five hues.** The five-hue mapping stays
 ...
 * heatmap and `score-tone.ts` read, so the two surfaces agree; the ramp is
 * monotonic, so Rough→Great carries an ordinal cue the five hues never did.
 *
        ...
        {counts.map((count, index) => {
          ...
          return (
            <View key={score} className="flex-row items-center gap-1.5">
              <View className={cn("h-2.5 w-2.5 rounded-[3px]", hueRampClass("be", score))} />

 ...
 *
 * Drop anchor writes a connection log with `technique: "dropAnchor"`, so it is not a
 * record type of its own — its entries reach history through this list (#1517).
 */
      ...
      <FlatList<ConnectionLog>
        ...
        ListHeaderComponent={
          <View className="mb-6 gap-6">
            <View className="gap-2">
              ...
              <ScreenHeader
                title={t("act:connection.listTitle")}
                right={<HelpButton helpKey="connection" />}
              />
              <Text variant="muted">{t("act:connection.listSubtitle")}</Text>
            </View>
            ...
            <Button onPress={() => pushWithOrigin("/modules/act/connection/new")}>
              ...
              <Text>{t("act:connection.newTitle")}</Text>
            </Button>
        ...
        ListEmptyComponent={
          isPending ? null : isError ? (
            ...
            <Text variant="muted">{t("act:connection.noLogs")}</Text>
          )
        }
        ListFooterComponent={
          isFetchingNextPage ? (
            ...
            <View className="flex-row items-start justify-between gap-2">
              <View className="flex-1 gap-1">
                <Text className="font-semibold leading-snug" numberOfLines={2}>
                  {log.noticesFromSenses || t(`act:connection.techniques.${log.technique}`)}
                </Text>
                <Text variant="muted" className="text-xs">
                  {t(`act:connection.techniques.${log.technique}`)}

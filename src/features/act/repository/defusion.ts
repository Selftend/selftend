...
/**
 * One page of every defusion log, newest first - the archive read behind `/modules/act/defusion` (#1517).
 *
 ...
 */
export async function listDefusionLogsPage(
  userId: string,
  limit: number,
  cursor: RecordCursor | null,
) {
  return selectList<DefusionLogRow, DefusionLog>((c) => {
    let query = c
      .from("act_defusion_logs")
      .select("*")
      .eq("user_id", userId)

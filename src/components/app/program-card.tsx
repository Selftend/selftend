...
const TINT_CLASSES: Record<
  ProgramTint,
  {
    ...
    startTitle: string;
    dismissActive: string;
    dismissIcon: string;
    eyebrowGlyphBox: string;
    routeIcon: string;
    manageActive: string;
    phaseTitle: string;
    readyBanner: string;
    readyText: string;
    taskRowDone: string;
    taskRowDoneIcon: string;
  }
> = {
  act: {
    startContainer: "gap-3 rounded-2xl border border-act/30 bg-act/5 p-5",
    startTitle: "flex-1 text-act",
    dismissActive: "-mr-2 -mt-2 size-9 items-center justify-center rounded-full active:bg-act/10",
    dismissIcon: "size-5 text-act",
    eyebrowGlyphBox:
      "h-[38px] w-[38px] items-center justify-center rounded-[10px] border border-[hsl(var(--act)/0.30)] bg-[hsl(var(--act)/0.12)]",
    routeIcon: "text-act",
  ...
  primary: {
    ...
    dismissActive:
      "-mr-2 -mt-2 size-9 items-center justify-center rounded-full active:bg-primary/10",
...
function TaskRow({ task, ns, tint }: { task: ProgramTaskView; ns: string; tint: ProgramTint }) {
  ...
  return (
    ...
    >
      <Icon
        ...
        className={task.done ? TINT_CLASSES[tint].taskRowDoneIcon : "size-5 text-muted-foreground"}
      />
...
export function ProgramCard({
  ...
  if (program.status === "not_started") {
    return (
      <View className={TINT_CLASSES[tint].startContainer}>
        <View className="flex-row items-start gap-3">
          <Text variant="h3" className={TINT_CLASSES[tint].startTitle}>
          ...
          {onDismissStart ? (
            <Pressable
              ...
              className={TINT_CLASSES[tint].dismissActive}
              role="button"
            >
              <Icon name="close" className={TINT_CLASSES[tint].dismissIcon} />
            </Pressable>
          ) : null}
        </View>
      ...
      <Card className="relative overflow-hidden gap-3 p-5">
        ...
        <View className="gap-2">
          ...
          <View className="flex-row items-center gap-3">
            <View
              ...
              className={TINT_CLASSES[tint].eyebrowGlyphBox}
            >
              <Icon name="route" size={22} className={TINT_CLASSES[tint].routeIcon} />
            </View>
            <Text variant="eyebrow" tint={tint} className="flex-1">
              {t("program.heroTitle")}
            </Text>
            {onAbandon ? (
              <Popover>
                <PopoverTrigger asChild ref={triggerRef}>
                  <Pressable
                    ...
                    accessibilityRole="button"
                    hitSlop={DEFAULT_INTERACTIVE_HIT_SLOP}
                    className={TINT_CLASSES[tint].manageActive}
                    role="button"
                  >
          ...
          <View className="flex-row flex-wrap items-baseline gap-2">
            <Text variant="h3" className={TINT_CLASSES[tint].phaseTitle}>
        ...
        <View className="gap-2">
          ...
          {phase.milestones.map((milestone) => (
            <TaskRow key={milestone.key} task={milestone} ns={ns} tint={tint} />
          ))}
        </View>
        ...
        {phase.dailyPractice ? (
          <View className="gap-2">
            ...
            <TaskRow task={phase.dailyPractice} ns={ns} tint={tint} />
          </View>
        ) : (
          <Text variant="muted" className="text-sm">
        ...
        {/* Ready banner */}
        {program.phaseReady ? (
          <View className={TINT_CLASSES[tint].readyBanner}>
            <Text className={TINT_CLASSES[tint].readyText}>{t("program.ready")}</Text>
          </View>
        ) : null}

        {/* Advance button */}
        <Button onPress={handleAdvancePress} disabled={isPending} variant="tinted" tint={tint}>

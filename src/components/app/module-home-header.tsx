...
interface ModuleHomeHeaderProps {
  ...
  variant?: "hero" | "field";
  /**
   * PROTOTYPE (#444): the owning scroll view's contentOffset.y. When set in
   * field mode, the field gradient translates at a fraction of scroll speed,
   * so the sheet rises over a background that lags it - the "subtle parallax"
   * candidate. The gradient is oversized upward so the lag never exposes an
   * edge. Ignored under reduce-motion.
   */
...
export function ModuleHomeHeader({
  ...
  // PROTOTYPE (#444). A reduced-motion preference pins the gradient still.

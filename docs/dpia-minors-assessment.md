...
data-protection assessment (Connecticut C.G.S. §§ 42-529 et seq., Colorado
SB 24-041)
that [spec #227](https://github.com/Selftend/selftend/issues/227) §4 requires
before Selftend admits anyone under 18. Written for
...
and are tracked as separate work (spec §10).
...
1. **The age gate** (`src/components/app/age-gate.tsx`, decided by
   ...
   country of residence chosen from a type-to-find list of all 250 ISO 3166-1
   codes. **Nothing before or during the questions names an age, a range, or a
...
2. **The consent gate** (`src/components/app/consent-gate.tsx`). Two separate
   ...
   or write to privacy@selftend.org) is stated beside the control.
...
The account that exists by then (three of the four entry paths create the auth
user before the gate runs) is deleted immediately through the self-service
`delete_user_account()` RPC, which delegates to `purge_user_account(uuid)`: the
person's avatar objects, then every per-table row, then the `auth.users` row. A
...
and non-shaming, says the account has been removed, and carries the two links
that work without an account — `/crisis` and Find A Helpline.
...
| Crash and error reports                                                                | Sentry                        | `sendDefaultPii: false`, plus `scrubEvent` / `scrubBreadcrumb` in `src/lib/sentry.ts`, which drop the email, console breadcrumbs and query strings |
...
Everything above is included in the JSON export a person can take from Settings
(`export_user_data()`), the three attestation columns among it — a country
someone typed and the timestamp of their own act are plainly their data.
...
| R6  | Engagement mechanics extend or compel a minor's use                                                               | Low        | High     | Principle 12 _Fulfilling, And Done_ and [ADR-0004](adr/0004-retention-by-return-not-engagement.md): nothing counts days or run-lengths, no levels or badges, nothing varies by date or visit, no contact triggered by non-use, reminders opt-in and off by default. Guarded by `test/practice-copy.test.ts` | Low      |
...
| No targeted advertising to a minor (categorical in CT since 2026-07-01)        | CT SB 1295; CO SB 24-041                             | No advertising of any kind, no ad SDK, no ad network, no advertising identifier. Ad-based monetisation is a standing product refusal, not a setting                                                                         | `AGENTS.md` product guardrails, [analytics.md](analytics.md)                                           |

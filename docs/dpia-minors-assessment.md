...
**Who is asked the age question.** Any account that has not already answered it
(`age_floor_met` is not `true`), guests included, **unless** it is exempt — and
the exemption has two halves, both required: the account was created before
`2026-09-05T00:00:00Z`, and it has already been through the consent gate
(`policy_version_accepted` is not `null`). People who were already here before
the gate existed and have consented are therefore not re-asked for an age or a
country; they meet the new consent step through the policy-version bump instead.
An old account that never accepted a policy version has been through neither
gate, so it is asked.

⚠️ **The exemption is not read off `policy_version_accepted` alone.** It was
until [#2227](https://github.com/Selftend/selftend/issues/2227): shipped 0.17.0
carries the consent wall and no age gate, so an account created on it records a
policy version and, on updating to a build with the gate, presented as an exempt
pre-gate account — permanently, since nothing writes that column back to `null`.
The account's own creation time is what separates "was already here" from
"arrived on a client that had no gate", and the cutoff is deliberately the
instant of the migration that added the column
(`20260905000000_age_attestation.sql`) rather than the release date, which a
client cannot know and which can slip. So **every account created on or after
2026-09-05 is asked, including the ones created on 0.17.0 in the days before the
release; if the release slips, that cohort grows.** Erring early costs a person
one birth year and one country. Erring late admits somebody below their floor
with no attestation on file, which is the harm the gate exists to prevent.

⚠️ The gate runs in the client. `age_floor_met` is a nullable column with no
default and no constraint, and nothing server-side enforces the floor or refuses
a write from an account that never attested; a build without the gate does not
apply it. This is attestation, as §7 and R2 say — not verification, and not a
database rule.
...
irreversible, so three things guard the path to it. An impossible date ("31
February") is caught by the calendar check _before_ the floor is measured and
shown as a correctable field error rather than an exit. The deletion is
confirmed rather than automatic
([#2193](https://github.com/Selftend/selftend/issues/2193)), on a screen that
says in as many words that removing the account deletes everything in it
permanently — the second net is on the destructive step, because a
plausible-but-wrong year is a real date the calendar check cannot catch. And in
the ordinary case the account holds nothing yet: it meets the gate before the
consent gate, on its first launch.

⚠️ **"Holds nothing yet" is no longer true of every account that meets the
gate.** Since [#2227](https://github.com/Selftend/selftend/issues/2227) the gate
also reaches accounts created on 0.17.0, which has no age gate — those people
have consented, used the app, and may hold entries by the time they first meet
...
[#2229](https://github.com/Selftend/selftend/issues/2229) closed both halves of
that window: an unknown attestation, errored or still in flight, now renders a
retry surface rather than the shell, so no content accumulates because the gate
has not run yet.
...
| R7  | The gate falls open while the attestation is unknown — the read errored, or is still in flight — and a person reaches the shell un-attested | Low        | Medium   | Closed in two halves. [#2200](https://github.com/Selftend/selftend/issues/2200) closed the errored one; [#2229](https://github.com/Selftend/selftend/issues/2229) closed the in-flight one, which #2200 had left open — a request still on the wire carried no error, so the shell rendered for as long as the fetch took, on every new account's first launch. Either way the app is now replaced by a surface that renders nothing below the floor. Failing closed does not re-ask anybody — the surface is not a gate, it asks nothing and records nothing — and it is not a lockout: the retry and TanStack's own refetch clear the errored half, and the in-flight half clears when the row arrives. [#2228](https://github.com/Selftend/selftend/issues/2228) put `/crisis` and Find A Helpline on both halves, so the block is never a dead end. Pinned by `protected-layout.test.tsx` and `preferences-unavailable-screen.test.tsx` | Low      |
...
and [#2229](https://github.com/Selftend/selftend/issues/2229) closed it — one
half each.
...
| Notifications and engagement contact only on an explicit opt-in                | The same duty, applied to reminders                  | Reminders are off by default, opt in per channel, and switch off in Settings; nothing is sent because someone stopped using the app                                                                                         | Principle 5, [architecture.md](architecture.md), [ADR-0010](adr/0010-one-reminder-is-suggested-by-placement-alone.md) |
...
R7 — the gate falling open while the attestation is unknown — was carried here
until [#2200](https://github.com/Selftend/selftend/issues/2200) closed the
errored half and [#2229](https://github.com/Selftend/selftend/issues/2229) the
in-flight one. It is no longer a residual risk: neither state renders the shell.
The objection that closing it would lock out every user whose first fetch failed
is answered by the shape of what replaces the shell — a screen with a retry asks
the person nothing and holds nobody permanently, where a _gate_ shown on the same
state would have re-asked somebody who had already answered — and by
[#2228](https://github.com/Selftend/selftend/issues/2228), which keeps `/crisis`
and Find A Helpline on it, so being held there never puts crisis guidance further
away than the build it replaces.

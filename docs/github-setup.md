...
| `EXPO_PUBLIC_PLAY_STORE_URL`            | optional             | n/a                  | n/a                 | Play listing URL: store chip and Android mobile-web download bar. Empty shows "Coming soon" and disables the bar. Set in prod+staging environments and as a repo var for the Android build. Note **native update offers are currently disabled** (the update banner is web-only), so this no longer affects any in-app update prompt. |
| `EXPO_PUBLIC_APP_STORE_URL`             | optional             | n/a                  | n/a                 | App Store URL shown on web surfaces; empty shows a "Coming soon" chip. It would also target the iOS update offer, but **native update offers are currently disabled** — the update banner is web-only, because `/version.json` carries a web timestamp rather than real store availability.                                           |

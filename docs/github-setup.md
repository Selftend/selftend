...
`Codex quota alarm` is scheduled daily (`codex-quota-alarm.yml`). It looks at the PRs merged into `dev` in the last 24 hours and files one `needs-triage` issue, "Codex reviewer is out of quota", when any of them carries a `chatgpt-codex-connector` comment saying it has reached its usage limits **and** no review from that account. It exists because that failure is silent: the reviewer posts a friendly comment, records zero reviews, and the merge proceeds, which is how 27 days and four releases went by with no code review at all ([#2221](https://github.com/Selftend/selftend/issues/2221)). It is **not** a required check and Codex is **not** a merge gate (owner ruling on [#2178](https://github.com/Selftend/selftend/issues/2178)); the job closes nothing, needs no secret beyond the workflow token, and updates its issue in place while it stays open. A closed issue counts as acknowledged only until another PR is starved after it, so closing it cannot mute the alarm.
...
| `EXPO_PUBLIC_SUPPORT_EMAIL`             | required (warns)     | required (warns)     | consumed            | Public support contact: the `/support` row and the FAQ's contact answer and parents letter, which interpolate it (#2131). "Validated" means a production build **warns** when it is unset, not that it fails — the copy falls back to `support@selftend.org` rather than publishing a policy with no contact in it. Unlike the store URLs below, an empty string is **not** an opt-out here; `/support` still hides its own row when the value is blank.                                                                                                                                        |
| `EXPO_PUBLIC_PRIVACY_EMAIL`             | required (warns)     | required (warns)     | consumed            | Public privacy contact, interpolated into the FAQ the same way. Same warn-and-fall-back rule as the support address — a fork that leaves it unset publishes **our** address as its controller contact, which is what the warning exists to catch.                                                                                                                                                                                                                                                                                                                                               |
| `EXPO_PUBLIC_SECURITY_EMAIL`            | required (warns)     | required (warns)     | consumed            | Public security contact: the `/security` page's mailto and the FAQ's contact answer. Same warn-and-fall-back rule.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `EXPO_PUBLIC_WEB_PUSH_VAPID_PUBLIC_KEY` | consumed (warns)     | n/a                  | n/a                 | The web reminder channel's public key. **No longer "deferred"** ([#2263](https://github.com/Selftend/selftend/issues/2263)): the value has been set on the `production` and `staging` environments since 2026-07-16, but `web-deploy.yml` did not forward it into the export env until that issue, so every deployed bundle inlined an empty key and browser reminders were dead while the variable looked configured. Unset, the app logs a startup warning on production web and web reminders stay disabled; native is unaffected. `test/web-deploy-public-env.test.ts` pins the forwarding. |
| `EXPO_PUBLIC_PLAY_STORE_URL`            | optional             | n/a                  | n/a                 | Play listing URL: store button and Android mobile-web download bar. **Defaults in code to the live listing**, so this var is an override; set in prod+staging environments and as a repo var for the Android build to keep the release env explicit. An empty string drops the button and the bar (the self-hoster affordance). Also targets the **Android** in-app update offer; empty disables that offer (never falls back to the other store).                                                                                                                                              |
| `EXPO_PUBLIC_APP_STORE_URL`             | optional             | n/a                  | n/a                 | App Store URL shown on web surfaces. **Defaults in code to the live listing**, so this var is an override; set in prod+staging environments and as a repo var (mirrors the Play var; set 2026-08-20 once the App Store listing went live). An empty string drops the button. It would also target the iOS update offer, but **iOS update offers stay disabled** — nothing on the device can observe an App Store promotion, so there is no availability signal to act on. Android does not have this problem: it asks Play directly.                                                            |

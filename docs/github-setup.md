...
| `EXPO_PUBLIC_PLAY_STORE_URL`            | optional             | n/a                  | n/a                 | Play listing URL: store chip, Android download bar, and the **Android** native update offer. Empty shows "Coming soon" and disables the bar and the Android offer. It does **not** affect iOS. Set in prod+staging environments and as a repo var for the Android build. |
| `EXPO_PUBLIC_APP_STORE_URL`             | optional             | n/a                  | n/a                 | App Store URL shown on web surfaces; empty shows a "Coming soon" chip. **Also drives the native iOS update offer** — while it is empty, an iOS build never offers an update (deliberately: it must never fall back to the Play listing).                                 |

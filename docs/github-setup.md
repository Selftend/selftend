...
| `EXPO_PUBLIC_PLAY_STORE_URL`            | optional             | n/a                  | n/a                 | Play listing URL: store chip and Android mobile-web download bar. Empty shows "Coming soon" and disables the bar. Set in prod+staging environments and as a repo var for the Android build. Also targets the **Android** in-app update offer; empty disables that offer (never falls back to the other store).                                                                                                                                                 |
| `EXPO_PUBLIC_APP_STORE_URL`             | optional             | n/a                  | n/a                 | App Store URL shown on web surfaces; empty shows a "Coming soon" chip. Set in prod+staging environments and as a repo var (mirrors the Play var; set 2026-08-20 once the App Store listing went live). It would also target the iOS update offer, but **iOS update offers stay disabled** — nothing on the device can observe an App Store promotion, so there is no availability signal to act on. Android does not have this problem: it asks Play directly. |

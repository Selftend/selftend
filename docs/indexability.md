...
**The word:** the site is made **readable** - it tells a crawler truthfully what each page already says - and never **pursued**. That distinction is the marketing plan's ([marketing-plan.md](marketing-plan.md) § 4, square 3, from [#2289](https://github.com/Selftend/selftend/issues/2289)), and it bounds this spec: **no choice made for a query.** ☠️ **It does not bound it by "no new copy, no new page"** — that clause described what this spec did, and [#2399](https://github.com/Selftend/selftend/issues/2399) ruled it **descriptive, not binding**. What separates readable from pursuit is **motive**, not the artifact: the plan's _The motive test_ (§ 4, square 3), and the spec it governs is [public-pages.md](public-pages.md).
...
- **`WebSite`:** `name` "Selftend" · `url` · `publisher` pointing at the Organization by `@id`. No `alternateName`, no `SearchAction` (the sitelinks search box was removed and the site has no search). ☠️ **Date correction ([#2400](https://github.com/Selftend/selftend/issues/2400)):** 2024-11-29 is when Google removed the **documentation**; the **feature** was removed starting **2024-11-21**, announced **2024-10-21**. The `SearchAction` decision is unaffected. `alternateName` is separately refused on [#2405](https://github.com/Selftend/selftend/issues/2405) — every on-site input is already correct, so it would change nothing.
...
- **A test pins** `Organization.description` to the rendered meta description and `logo` to `og:image`, so the block cannot disagree with the visible page. Validation in the Rich Results Test or Search Console's _Enhancements_ is a check, not a goal - no rich result is expected. ☠️☠️ **The win this line used to claim — "the logo and site-name association" — is inert, on both counts** ([#2401](https://github.com/Selftend/selftend/issues/2401), corrected on [#2405](https://github.com/Selftend/selftend/issues/2405)): the `logo` effect only selects _which_ logo inside a knowledge panel, and **no on-site-only path to a panel exists**; the site name still renders as the bare domain `selftend.org` while **every documented on-site input is already correct**, leaving only off-site references, which the plan refuses. **The block stays** — it is truthful, free, parses with 0 errors and Bing reads it — but its justification is **truthfulness alone**, and **no property here may ever be justified by expected effect** ([public-pages.md](public-pages.md) § 5, which adds the only two properties that passed). Bing reads JSON-LD; nothing engine-specific to add.

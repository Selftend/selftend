...
**Status:** Decided spec, assembled 2026-09-10 from wayfinder map [#2281](https://github.com/Selftend/selftend/issues/2281) on its last ticket, [#2290](https://github.com/Selftend/selftend/issues/2290). Every section links the ticket whose resolution comment holds the full reasoning and the owner's ruling. **Built so far:** § 10 items 1 and 2 - the two landing keys, the static export with `app/+html.tsx`, the root layout's site-default `<Head>` as the sole `lang` owner, and the landing's own `<Head>` (§ 2, § 4.1 for `/`, § 4.2-4.4) - on [#2293](https://github.com/Selftend/selftend/issues/2293); item 3, the per-route heads (§ 4.1-4.5, § 7's title = H1 test, the not-found file's title and `noindex`), on [#2294](https://github.com/Selftend/selftend/issues/2294), so the stopgap is gone and the root `<Head>` carries only the site constants; and items 5 and 6 - the index list in `scripts/lib/index-list.js`, the prune to the nine files, `sitemap.xml` from the same list, `public/robots.txt`, the staging strip and `not_found_handling = "404-page"` in both Worker configs (§ 3, § 6.4-6.7, the three new tests of § 7) - on [#2295](https://github.com/Selftend/selftend/issues/2295); and item 4, the structured-data block in the landing `<Head>` - `src/lib/structured-data.ts` read by `LandingHead`, its pinning test and the one-surface gate `test/structured-data-surface.test.ts` (§ 5, § 7) - on [#2296](https://github.com/Selftend/selftend/issues/2296). Item 7, the docs, is on [#2297](https://github.com/Selftend/selftend/issues/2297), the ticket that also carries the release (item 8) and the owner steps of § 9: §§ 2-6 are **as built** - released as v0.19.0 on 2026-09-10, with the zone settings of § 6.1 and § 6.2 live from 2026-09-11 - and the zone settings of § 6.1, § 6.2 and § 6.5 are recorded in [deployment.md](deployment.md) § _Cloudflare zone settings_, each row with the `curl` that verifies it and the date it was last verified; the control-tower issue of § 6.8 is [control-tower #132](https://github.com/vasilyoshev/control-tower/issues/132). Item 9 is built on [#2298](https://github.com/Selftend/selftend/issues/2298): `applinks:www.selftend.org` left `ios.associatedDomains`, and the `/.well-known/` carve-out in the redirect rule is retired by the owner once a build without the claim is the current App Store version (the runbook's one-time step; the date lands in the deployment table). `/to-tickets` sliced § 10 into implementation tickets ([#2293](https://github.com/Selftend/selftend/issues/2293)-[#2298](https://github.com/Selftend/selftend/issues/2298)); this file is the as-built reference for the public website's index, and the standing operational parts are repeated in [deployment.md](deployment.md).
**Audience:** Developers and product contributors; the owner for the dashboard steps in § 9.
**The word:** the site is made **readable** - it tells a crawler truthfully what each page already says - and never **pursued**. That distinction is the marketing plan's ([marketing-plan.md](marketing-plan.md) § 4, square 3, from [#2289](https://github.com/Selftend/selftend/issues/2289)), and it bounds this spec: no choice made for a query. What separates readable from pursuit is motive, not the artifact - the plan's _motive test_, from [#2399](https://github.com/Selftend/selftend/issues/2399); this spec _wrote_ no new copy and _created_ no page, which describes what it did and bounds nothing that comes after it. What comes after it is [brand-result.md](brand-result.md) (decided, not built): explainer pages from content the app already holds, one footer linking every public page, two more structured-data properties - it extends § 3, § 4.1, § 5 and § 7 here and adds internal linking, and it re-litigates nothing.
...
- **`WebSite`:** `name` "Selftend" · `url` · `publisher` pointing at the Organization by `@id`. No `alternateName`, no `SearchAction` (Google removed the sitelinks search box from Search from 2024-11-21, announced 2024-10-21, and removed its documentation on 2024-11-29 - this spec first recorded the documentation date as the feature's ([#2400](https://github.com/Selftend/selftend/issues/2400)); and the site has no search).
- **Never**, each a decision: `nonprofitStatus` (no registered entity exists - `docs/costs.md`, checked 2026-08-20 - so a schema.org `NonprofitType` would be a false legal claim); a `founder` or `Person` node; `email` or `contactPoint`; the store URLs; anything implying outcomes, ratings, reviews or a category noun positioning refuses. Added on [#2405](https://github.com/Selftend/selftend/issues/2405), each with its reason: `knowsAbout` (a list of therapy names no page displays - keyword-chosen copy in structured-data clothing); `foundingDate` (no page displays a date; the only defensible value is the first commit, a fact about a repository, and it would sit beside a `nonprofitStatus` refused for having no legal entity); `alternateName` (every documented on-site input for the site name is already correct and the name still renders as the bare domain, so it would change nothing - and there is no true alternative name). `BreadcrumbList` on any page: the public routes are flat, so there is no hierarchy to describe, and breadcrumbs are not documented to feed sitelinks ([#2400](https://github.com/Selftend/selftend/issues/2400)).
- **Refused types, so nobody re-derives them:** `FAQPage` and `SoftwareApplication` are refused because **no documented reader acts on them and Selftend gains nothing truthful by asserting them** - not merely because a rich result went away, which would imply the refusal expires if Google reinstated one; it does not ([#2405](https://github.com/Selftend/selftend/issues/2405), restating [#2291](https://github.com/Selftend/selftend/issues/2291)). The facts that first decided them still hold: Google stopped showing the FAQ rich result on 2026-05-07 and removed its documentation on 2026-06-15; `SoftwareApplication`'s rich result requires `aggregateRating` or `review`, which Selftend has none of and may not invent, and its `applicationCategory` would force a category noun positioning refuses. No health or medical type exists in Google's gallery, and claiming one walks into the "not a diagnosis engine" guardrail.
...
- **A test pins** `Organization.description` to the rendered meta description and `logo` to `og:image`, so the block cannot disagree with the visible page - and the same rule binds **any block on any page**: it may only restate strings its own page renders ([#2405](https://github.com/Selftend/selftend/issues/2405); a test can pin a string and cannot pin a judgement). Validation in the Rich Results Test or Search Console's _Enhancements_ is a check, not a goal - no rich result is expected. ☠️ **This spec first said the win was "the logo and site-name association"; both halves are inert** ([#2401](https://github.com/Selftend/selftend/issues/2401)): the `logo` property is documented to influence _which_ logo shows inside a knowledge panel, and no panel exists or can be created on-site; the site name still renders as the bare domain while every documented on-site input is already correct. **The block is kept because it is truthful, costs nothing and parses clean, with no observable effect today** - and so no property may ever be justified here by an expected effect ([brand-result.md](brand-result.md) § 6). Bing reads JSON-LD; nothing engine-specific to add.
...
### 6.3 The app-links claim on `www` - dropped, the carve-out retired once that build is current

- **While a build claiming `www` is still the current App Store version:** the rule spares `/.well-known/*` on `www`, so `apple-app-site-association` keeps answering 200 there. Necessary, not cautious: Apple requires the file served "with no redirects" ([Supporting associated domains](https://developer.apple.com/documentation/xcode/supporting-associated-domains)); Google's Digital Asset Links says "301 or 302 response codes are not followed" ([Create a statement](https://developers.google.com/digital-asset-links/v1/create-statement)). Android is unaffected either way: its intent filter in `app.config.ts` names the apex host only.
- **Built on [#2298](https://github.com/Selftend/selftend/issues/2298):** `applinks:www.selftend.org` left `ios.associatedDomains` in `app.config.ts`; the first native release after it ships the apex-only entitlement. The carve-out is retired by the owner **once that build is the current App Store version** - the one-time step in `docs/launch/app-links-runbook.md` § _Retiring the `www` carve-out_, which ends by dating the redirect-rule row in `docs/deployment.md` and updating control-tower #132. A build still carrying the old entitlement loses nothing meanwhile: a tapped `www` link opens Safari, follows the 301, and the apex web app completes `/auth-callback` as it does today; a tapped link with the app installed never touches the website at all.
- The same ticket rewrote `docs/launch/app-links-runbook.md` (which said "both hosts serve the app directly rather than redirecting") and the matching comment above `associatedDomains` in `app.config.ts`: both now state the apex as the only origin and why.
...
`docs/deployment.md` carries a **"Cloudflare zone settings"** section: the redirect rule (expression, target, status), Always Use HTTPS, the managed robots.txt toggle - each with the date verified. A **control-tower issue** is filed by the build per the architecture rule (a redirect rule changes a public URL; the `www` claim's retirement changes the app-links inventory).
...
1. Cloudflare → Rules → Redirect Rules: the `www` → apex rule of § 6.1 (verify with `curl -I https://www.selftend.org/faq` → 301 to the apex, and `curl -I https://www.selftend.org/.well-known/apple-app-site-association` → 200 for as long as the carve-out of § 6.3 stands; 301 once it is retired).
...
9. **The `www` app-links claim** - `applinks:www.selftend.org` leaves `ios.associatedDomains`; `docs/launch/app-links-runbook.md` and the config comment rewritten (§ 6.3). Built on [#2298](https://github.com/Selftend/selftend/issues/2298); the `/.well-known/` carve-out is retired from the redirect rule by the owner once the first native release carrying it is the current App Store version (the runbook's one-time step).

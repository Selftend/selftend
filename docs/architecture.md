...
- `src/stores/theme-store.ts` persists the choice. It also exposes `hydrated`: once true, `hydrate()` neither re-reads storage nor applies a read already in flight, so an explicit or account-synced choice always beats disk.

...
✅ **The frame-spelling invariant ([#1627](https://github.com/Selftend/selftend/issues/1627)) is the second growth ring, and the last one this document promised.** The product spelled its own defining word two ways — nine British against four American, twice inside `cbt.json`, and with "Behavioural activation" and "Behavioral activation" reachable in one session. British wins because § _Words to use_ says so. The four American strings were respelled and the rule landed in the same change, on the same bargain as the first: fix the copy, or do not add the rule.

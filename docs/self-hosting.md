...
Store links: `EXPO_PUBLIC_PLAY_STORE_URL` and `EXPO_PUBLIC_APP_STORE_URL` are shown on web surfaces. The Play URL additionally drives the Android mobile-web download bar and the **Android** native update offer; the App Store URL drives the **iOS** one. Each native platform uses only its own store URL and never falls back to the other, so an empty App Store URL means iOS offers no update at all. Leave both empty on a self-hosted fork - your build is not on our listings. Empty means the "Coming soon" chip, no download bar, and no native update offer; the web update banner still works because it reads `/version.json` from your own origin (write it in your deploy step, or skip it and the check stays silent).

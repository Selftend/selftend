...
**Every user-visible string in the app**: all 20 i18n namespaces, `en` and `bg` — 4,140 English strings, ~156,000 characters. Namespaces are the honest unit here, because that is where all user-visible copy lives (AGENTS.md § i18n conventions makes hardcoded strings a test failure), so covering all 20 covers every module by construction.
...
| ------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | -------------------- | ---------------------- | ------------------- | --------- | -------- |
| CBT module — Think/Act/Be, thought records, distortions, beliefs, worry, anger, procrastination, activities, recovery, grounding, breathing | `cbt`                       | **Fixed**            | **Fixed** + **Raised** | Pass                | **Fixed** | Pass     |
...
`test/child-safety-copy.test.ts` carries the rules this review established, as a merge gate. Eleven of its rules were red against live copy before the fixes above.

☠️ **Every rule is keyed on a claim shape, never on subject matter.** Selftend is a mental health app; it has to be able to say "anxiety", "depression", "panic", "crisis", and "not a cure", and the crisis page's whole job is to say some of them plainly. A rule keyed on the vocabulary would go red on the copy that protects people — the over-sweep failure that gets a guard deleted instead of fixed. Seven legitimate neighbours are pinned in the same file as must-stay-legal for that reason.

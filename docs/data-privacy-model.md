...
| Enabled modules         | `user_preferences.enabled_modules`                                          | Written (default `['cbt']`, plus the meditation wizard), read by nothing: no screen gates on it (#1672). If it ever gates again, use explicit user choice and never silently enable a module.                                                       |
| Age attestation         | `user_preferences.age_floor_met`, `age_attested_country`, `age_attested_at` | Store the age gate's **verdict**, never the date of birth: the DOB is compared against the country's floor and discarded (#1762, spec #227 §3). `null` means never asked, which is not the same as failing. Exported, and deleted with the account. |
...
| Notifications           | Preferences, push channel registrations                                     | Explicit, quiet by default, easy to disable, and never exposing sensitive content.                                                                                                                                                                  |
...
| Device push tokens      | `device_push_tokens`                                                        | Store only the Expo push token, platform, timezone, and ownership. Delete when the global master goes off, on sign-out, and on account deletion.                                                                                                    |
...
- User-chosen occurrence time must remain separate from immutable creation and update audit timestamps. The mood, gratitude, sleep, and journal records also store the device's original UTC offset; existing rows use offset `0` where it cannot be recovered.

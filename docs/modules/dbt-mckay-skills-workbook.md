...
- **`ModuleKey: "dbt"`** in the union and `VALID_MODULES`. (The spec also named a local shadow in `modules-screen.tsx`; that screen was deleted with the `/modules` page in [#2114](https://github.com/Selftend/selftend/issues/2114), so there is no shadow left to keep in step.) Default `enabledModules` stays `["cbt"]` and gates nothing.

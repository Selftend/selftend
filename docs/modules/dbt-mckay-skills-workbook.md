...
- **Where it is set:** the reminder bell on the DBT module home, and Settings › Reminders. Nothing offers it after a save — the post-completion card that once did was removed outright (ADR-0008), so the capability is found rather than proposed. Off by default, time-of-day trigger only, one explicit choice. ⚠️ DBT is also held out of delivery until the native build routing `/modules/dbt` is live on both stores (`reminder-rollout.ts`).

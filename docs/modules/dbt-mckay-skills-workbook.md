...
**Status:** Canonical spec for the DBT module, assembled 2026-09-05 from wayfinder map [#1980](https://github.com/Selftend/selftend/issues/1980) - every decision below links to the ticket whose resolution comment holds its full reasoning. **The module is built.** The first-release slice named in §2 - the seven native tools and the one timed session, on nine routes' worth of surfaces, plus the learn pages and the four-phase programme - shipped across [#2024](https://github.com/Selftend/selftend/issues/2024)-[#2033](https://github.com/Selftend/selftend/issues/2033) and has been in production since v0.18.0 (2026-09-09). The second slice (§3.5) and the post-MVP sections (§3.6) stay specified and unscheduled (§13). The companion design brief was `docs/design/1980-handoff/`; what it returned opened the implementation map that cut those tickets.
...
- **Done:** states the record - _Muscle relaxation, 14 minutes_ - one **Done** back to the module home. No rating, no _how do you feel_, and since ADR-0008 no reminder offer either: the completion moment carries no ask at all.
...
**Record:** `{ judgement, valence: negative | positive, restatement?, createdAt, createdOffsetMinutes }` (the ticket's `noticedAt` collapsed into `createdAt` - the record is not back-datable). History: grouped by day, the mark as a small glyph, **no counts** (never _you caught 12 this week_). The detail offers one door - **Unhook from it** → ACT defusion with the judgement seeded as the fused thought and `thoughtCategory: selfJudgment` preset through the ACT draft store (neither the ACT form nor the journal takes a query parameter); a defusion entry already held there for _Finish later_ wins over the seed, the same precedence the emotion record's door gives a live thought-record draft. Routine-steppable (`judgement`). UI spelling **Judgement** everywhere; identifiers may keep either.
...
- **Where it is set:** the reminder bell on the DBT module home, and Settings › Reminders. Nothing offers it after a save — the post-completion card that once did was removed outright (ADR-0008), so the capability is found rather than proposed. Off by default, time-of-day trigger only, one explicit choice. ✅ DBT's reminder was held out of delivery until the native build routing `/modules/dbt` was live on both stores; that build is v0.18.0 and the hold-out was lifted on 2026-09-22 (#2698), so the reminder now delivers like any other (`reminder-rollout.ts`).
...
| **`dbt_opposite_action_plans`** - open until done             | `pull` (≤500) · `opposite_action` (1–500, required) · `hold_for` (≤120) · `what_shifted` (≤1000)                                                                                                                                                                                                                            | `emotion text not null` · `done_at timestamptz null` · `done_offset_minutes`                                                                                                                                                                                                                                                                                                                                                    | `created_at` for the list; **`done_at` is the record's day** for the programme and routines       |
...
- **Looking back:** left the product 2026-09-15 ([#2431](https://github.com/Selftend/selftend/issues/2431)). The six dated tables keep their captured offsets and `occurrence_day_key` days for the programme and routines; `record_days` **was dropped on 2026-09-22** ([#2459](https://github.com/Selftend/selftend/issues/2459)), once the Play release carrying the removal was `Published`, and no DBT mutation invalidates a record-days root.
- **Routines - six steppable tools:** `SteppableToolId` gains `muscleRelaxation` (a `dbt_sessions` row with that slug on its completed day), `wiseMind`, `judgement`, `emotionRecord` (created day), `oppositeAction` (**done** day, list-first route), `script` (created day). Copies: `derive.ts` (type, `STEPPABLE_TOOL_IDS`, `RoutineToolRecords` slices with `dayKey`, `stepDoneOnDate`), `tool-routes.ts`, `use-routine-tool-records.ts`, the editor's grouping + `routines.json` `STEP_TOOL_GROUPS.dbt` in both locales, `starter-offer.ts` if DBT records should compose a starter. The seed's `ROUTINE_STEP_SOURCES` gains entries only if a seeded routine points at DBT - none does. ☠️ **The six are read-only for now (#2203):** a routine step is written by one client and read by another - the web build ships at merge, the store builds days later - so a DBT step composed on the web is unreadable, un-tickable and route-less on a phone still on the previous release. They are therefore listed in `WITHHELD_STEP_TOOL_IDS` (`src/features/routines/step-tool-rollout.ts`) and absent from the `routine_steps_tool_id_allowlisted` CHECK, so neither the editor's chips nor the starter can compose one. Widening is a second release once the native build is live on both stores: drop them from the withheld list **and** add them to the allowlist in a NEW migration, together - `step-tool-rollout.test.ts` fails if only one half lands. **Done on 2026-09-22 ([#2713](https://github.com/Selftend/selftend/issues/2713)):** the reading build is v0.18.0, live on both stores since well before, so the withheld list is empty and `20260922000000_routine_step_tool_allowlist_dbt.sql` carries the widened CHECK. The six are ordinary steppable tools now - offered by the editor, composable by the starter (last, after every ACT exercise).
...
The home row, the list h1 and the breadcrumb share the name; the record noun lives in body copy (_New script_, _Your scripts_). Header stats: `home.statRecords_one/_other`, `home.statSessions_one/_other`, `home.statLoadingValue` "—". Navigation: `sidebar.dbt` _DBT_ (kept - it labels the breadcrumb and the shared-tool entries, not a panel row); ☠️ **do not ship `sidebar.dbtA11y`** - an earlier draft of this line called for it, and [#2106](https://github.com/Selftend/selftend/issues/2106) deleted it along with the module rows it belonged to, so re-adding it recreates an orphan; `today.modules.dbtName` kept; **`today.modules.dbtSub` → _For when feelings run high_** (the modules tile and the favourites card). Help entries (what/how/why, no image): `dbtProgram` plus the eight tools; the _why_ states what the skill is for, never an outcome.
...
- **The child-safety review re-run was done** - seven passes on 2026-09-05, recorded in `docs/child-safety-review.md`, the last of them closing at _"the DBT module's first-release copy is fully reviewed"_ in both locales, plus a footnote covering the six DBT-named strings that live in `routines` rather than `dbt`. ⚠️ Two child-safety obligations are still open and neither is this one: a re-run over the post-MVP exposure sections' copy, recorded before their build ticket is cut (condition 3 of the four above), and the **IARC questionnaire re-run** owed at the next store submission, because this module's substance referral line made the app's answer name two substance references rather than one (`docs/child-safety-review.md` § _For the Play pass_). The string set the passes covered: the whole `dbt` namespace, `copy.dbt.*`, the DBT `navigation` and `help` keys - and specifically the coping-plan menu copy, the per-emotion hints, the sleep-hygiene list, the _You are allowed to_ lines, the push-back lines, the _what got in the way_ list and the reminder string, each of which also runs `positioning-copy`, `child-safety-copy`, `restraint-copy` and `practice-copy` in both locales.
...
| The final coping-plan pick set (≈66 lines) and the Remind-myself lines - written through the child-safety and positioning gates.                                | **Answered** - written through the child-safety and positioning gates and shipped in [#2027](https://github.com/Selftend/selftend/issues/2027)                                                |
| The six rail captions at 360dp on the emotion record form (the thought record's are one word each; these run to three).                                         | **Still open.** The design brief answered it, but map [#1980](https://github.com/Selftend/selftend/issues/1980) closed without recording a check against the shipped form; no ticket holds it |
| Whether `derive-<module>-program.ts` / `use-<module>-program.ts` generalise across three modules or fork a third time.                                          | **Answered: forked a third time.** `derive-dbt-program.ts` and `use-dbt-program.ts` ship beside the CBT and ACT pair ([#2032](https://github.com/Selftend/selftend/issues/2032))              |
| Whether DBT records compose a routine starter (`starter-offer.ts`).                                                                                             | **Answered: yes.** Six DBT record kinds feed `starter-offer.ts` ([#2033](https://github.com/Selftend/selftend/issues/2033))                                                                   |
...
| The sibling module h1 casing (CBT and ACT title case vs DBT sentence case).                                                                                     | **Still open**, as an observation - not a redesign. Map [#1980](https://github.com/Selftend/selftend/issues/1980) closed without recording it; no ticket holds it                             |

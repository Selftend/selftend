...
- one-page CBT onboarding, tracked in account preferences (the concern→strategy personalization scaffolding - `cbtWizardCompleted`, `active_strategies` - is intentionally retained for a deferred personalization step, not yet surfaced in a wizard; mirrors the deferred ACT wizard. `selected_concerns` was part of it until #1958 dropped the column with the one-panel app introduction)
  - ☠️ its conditions table is **two columns by decision, not by omission** (#1867): `Condition` + `CBT Focus`, with the condition spelled out in full and no "Core Feature" column. A third column pairing a named diagnosis with its defining feature turns background reading into a self-identification prompt, and this screen is met on first entry to the module by a readership that starts at thirteen (#1767). Neither younger module reaches for this shape - ACT ships a four-step onboarding with no conditions table, and DBT ships no intro keys at all - so a later module built from this one as a template must not re-import the third column. The "not a diagnosis, an assessment, or a treatment plan" clause is load-bearing and stays verbatim in both locales

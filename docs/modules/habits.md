...
| Phase                                           | Scope                                                                                                                                                                                                                         | Notes                                                            |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| **0 — Spec & plan**                             | This document. Data shape finalised. Asset paths decided. Onboarding copy drafted in both languages.                                                                                                                          | Done when this file is merged.                                   |
| **1 — Foundation**                              | DB migration for `habits` + `habit_logs` + `habitsOnboardingCompleted`. `habits` i18n namespace. Onboarding modal (4 steps). `?` re-open icon. Home with new-habit CTA, tap-to-tick, week strip, recent activity. Build-only. | Replaces the `/tools/habits` placeholder. Acceptance bar in §12. |
| **2 — Notes & history**                         | Optional note per tick. `/tools/habits/history` chronological list. Edit and archive flows. Calendar heat-strip on detail.                                                                                                    | Strict ordering by `logged_on`, then `created_at`.               |
| **3 — Inverse laws (break a habit)**            | Add `kind = 'break'` variant on the form. Inverse-law prompts (Make It Invisible / Unattractive / Difficult / Unsatisfying). Detail shows "days avoided" only if user opts in.                                                | Inverse laws come from the second half of the book.              |
| **4 — Learn cards**                             | Learn-card carousel on home. Individual cards for: Compounding, Habit Loop, Four Laws (one per law), Two-Minute Rule, Never Miss Twice, Identity-Based Habits, Systems Over Goals.                                            | Cards cycle locally; dismissal is session-only.                  |
| **5 — Insights**                                | Weekly rhythm chart, identity round-up, two-minute floor adoption. All private and on-device-derived.                                                                                                                         | Deferred — same posture as gratitude insights.                   |
| **6 — Reminders (out of scope until reviewed)** | If approved later, off by default; piggyback on the existing reminder-consent gate.                                                                                                                                           | Explicitly _not_ part of Phase 1–5.                              |

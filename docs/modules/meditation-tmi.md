...
- Routine builder: preferred time of day and preferred duration. The reminder itself is never offered - it is set from the reminder bell on the meditation home or from the reminders screen, not inside the builder (ADR-0008).
...
- **Sound door:** Beneath `Pause` / `Finish early`, a plain `Sound` button opens the sound panel, where the background bed and its volume can be changed without leaving the sit. Every pick applies live and the clock does not pause — auditioning under the running clock is the point. The door says the same word whether a bed is playing or not; the rules are `docs/sound.md` §1.

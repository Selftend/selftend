...
## Post-release: lift held-out entries

**Something can be withheld from everyone on purpose, and only a person lifts it.** Two constants do that today, for the same underlying reason — the shipped native client is older than the database and the web client, so a thing the new build understands can reach a phone that does not:

- **`HELD_OUT_REMINDER_TARGETS`** (`src/features/notifications/reminder-rollout.ts`) — reminder targets whose deep link the SHIPPED native client cannot route yet ([#2213](https://github.com/Selftend/selftend/issues/2213)). While a target is on that list the edge function mints nothing for it, and its row on the reminders screen is switched off under a "not ready yet" note ([#2260](https://github.com/Selftend/selftend/issues/2260)). (#2260 also suppressed the post-save reminder offer for a held-out target; that offer has since been removed for every target — ADR-0008.)
- **`WITHHELD_STEP_TOOL_IDS`** (`src/features/routines/step-tool-rollout.ts`) — steppable tool ids a routine step may not be WRITTEN with yet ([#2203](https://github.com/Selftend/selftend/issues/2203)). The read vocabulary and the write vocabulary are deliberately not the same set; a row composed on the web and read by a phone that has never heard of the id throws from a press handler and pins the routine FAB open. Its lift has a second half the reminder one does not — a migration widening the database CHECK, which `step-tool-rollout.test.ts` requires to land in the same change.

☠️ **Nothing lifts either one automatically, and until 2026-09-22 nothing went red while an entry stayed held out — which is exactly how a hold-out outlives its reason.** It did, three times over: `dbt` sat **13 days** past its own condition, the six DBT step tools **13**, and `general` **5**, across 32 releases in which neither list had ever been emptied. ⚠️ **Amended, not retired**: two guards now exist (below), and they close "invisible" and "unchecked" — they do not lift anything, and no mechanism will. The sentence above is the reason they are there, so do not tidy it away.

**Check after every release that carried a new held-out entry, once the build is live on BOTH stores.** Android is Play review; iOS needs the manual App Store Connect promotion described in [How iOS reaches users](#how-ios-reaches-users), and until that promotion happens iOS users stay on the old client indefinitely — so the store listings, not the merge, are what says the client is out there.

The lift is one edit plus its tests, in one change:

1. drop the entry from its constant — `HELD_OUT_REMINDER_TARGETS` in `src/features/notifications/reminder-rollout.ts`, or `WITHHELD_STEP_TOOL_IDS` in `src/features/routines/step-tool-rollout.ts`;
2. update the tests that name it. For a reminder target: `src/features/notifications/reminder-rollout.test.ts` and `supabase/functions/_shared/web-reminders.test.ts` (it pins the hold-out and the partition); `test/check-in-route-compat.test.tsx` asserts the same partition from the app side, and `src/features/notifications/notification-target-row.test.tsx` drives the held-out row from a mocked predicate so that path keeps its coverage while the list is empty. For a step tool id: `src/features/routines/step-tool-rollout.test.ts`, **plus a migration widening the database allowlist in the same change** — that test fails when the two disagree, so neither half can ship alone;
3. **mark it lifted in the table below, in the same change.** `test/holdout-table.test.ts` fails when a constant and that table disagree in either direction, so the edit and the record cannot drift apart;
...
5. merge — for a reminder target the release pipeline redeploys the edge function (`supabase functions deploy`) and both clients read the same file; for a step tool id the migration deploys with the release.

### The hold-out table

**Today: both constants are empty.** Emptied on 2026-09-22 by [#2698](https://github.com/Selftend/selftend/issues/2698) and [#2713](https://github.com/Selftend/selftend/issues/2713) — the first lifts either list has ever had.

This table is the register, not a summary. A new held-out entry belongs here in the same change that adds it, with the release it shipped in and the condition that lifts it, so the next reader can settle it by opening two store pages. **An empty _Lifted_ cell means still held out**, and both guards below read that column.

<!-- holdout-table:start -->

| Constant                    | Entry              | Withholds                             | Shipped in          | Lifts when                                                         | Lift issue                                                | Lifted                                                                |
| --------------------------- | ------------------ | ------------------------------------- | ------------------- | ------------------------------------------------------------------ | --------------------------------------------------------- | --------------------------------------------------------------------- |
| `HELD_OUT_REMINDER_TARGETS` | `dbt`              | the DBT module's daily reminder       | v0.18.0, 2026-09-09 | `/modules/dbt` in `ALLOWED_REMINDER_ROUTES` is live on both stores | [#1980](https://github.com/Selftend/selftend/issues/1980) | 2026-09-22, [#2698](https://github.com/Selftend/selftend/issues/2698) |
| `HELD_OUT_REMINDER_TARGETS` | `general`          | the one general daily reminder        | v0.21.0, 2026-09-16 | `/` in `ALLOWED_REMINDER_ROUTES` is live on both stores            | [#2494](https://github.com/Selftend/selftend/issues/2494) | 2026-09-22, [#2698](https://github.com/Selftend/selftend/issues/2698) |
| `WITHHELD_STEP_TOOL_IDS`    | `muscleRelaxation` | writing that tool into a routine step | v0.18.0, 2026-09-09 | `/modules/dbt/*` is live on both stores                            | [#1980](https://github.com/Selftend/selftend/issues/1980) | 2026-09-22, [#2713](https://github.com/Selftend/selftend/issues/2713) |
| `WITHHELD_STEP_TOOL_IDS`    | `wiseMind`         | writing that tool into a routine step | v0.18.0, 2026-09-09 | `/modules/dbt/*` is live on both stores                            | [#1980](https://github.com/Selftend/selftend/issues/1980) | 2026-09-22, [#2713](https://github.com/Selftend/selftend/issues/2713) |
| `WITHHELD_STEP_TOOL_IDS`    | `judgement`        | writing that tool into a routine step | v0.18.0, 2026-09-09 | `/modules/dbt/*` is live on both stores                            | [#1980](https://github.com/Selftend/selftend/issues/1980) | 2026-09-22, [#2713](https://github.com/Selftend/selftend/issues/2713) |
| `WITHHELD_STEP_TOOL_IDS`    | `oppositeAction`   | writing that tool into a routine step | v0.18.0, 2026-09-09 | `/modules/dbt/*` is live on both stores                            | [#1980](https://github.com/Selftend/selftend/issues/1980) | 2026-09-22, [#2713](https://github.com/Selftend/selftend/issues/2713) |
| `WITHHELD_STEP_TOOL_IDS`    | `checkTheFacts`    | writing that tool into a routine step | v0.18.0, 2026-09-09 | `/modules/dbt/*` is live on both stores                            | [#1980](https://github.com/Selftend/selftend/issues/1980) | 2026-09-22, [#2713](https://github.com/Selftend/selftend/issues/2713) |
| `WITHHELD_STEP_TOOL_IDS`    | `selfSoothe`       | writing that tool into a routine step | v0.18.0, 2026-09-09 | `/modules/dbt/*` is live on both stores                            | [#1980](https://github.com/Selftend/selftend/issues/1980) | 2026-09-22, [#2713](https://github.com/Selftend/selftend/issues/2713) |

<!-- holdout-table:end -->

Every lift above was made against the same reading of the stores: **App Store 0.21.0** (released 2026-09-17) and **Google Play 0.23.0** (updated 2026-09-18), with `eas.json`'s production submit profile on `releaseStatus: "completed"`, so there was no staged rollout to wait out.

### The two guards

Built on [#2717](https://github.com/Selftend/selftend/issues/2717), decided on [#2701](https://github.com/Selftend/selftend/issues/2701). ⛔ **Neither one lifts anything.** A lift is a judgement about a shipped client, and it stays a person's.

- **`test/holdout-table.test.ts`, in `verify` — closes "invisible".** Every entry of both constants must appear in the table above with a release and a condition, and every row with an empty _Lifted_ cell must still be in its constant. An equality in both directions, so neither an undocumented hold-out nor a row marked lifted while the entry is still withheld can pass. Purely local: no network, no issue state, no clock.
- **[`holdout-alarm.yml`](../.github/workflows/holdout-alarm.yml), weekly — closes "unchecked".** Reads the two live store versions and goes red when a still-held-out row's _Shipped in_ version is at or below both — that is, when its condition is met and nobody has looked. The comparison lives in `scripts/check-holdout-versions.mjs` and is unit-tested in `verify`, because an untestable comparison inside a weekly guard is how a guard starts lying without anyone noticing.

⛔ **An entry → open-issue binding was refused** on #2701, against the record: `general` had an open, correctly-labelled tracking issue ([#2494](https://github.com/Selftend/selftend/issues/2494)) and still overran its condition by 5 days, so that guard would have been green throughout the very failure it was written for. It also needs issue _state_ at merge time — a remote read, which this repo's precedent puts in an alarm rather than a gate — and a row naming a **closed** issue would pass a local check forever, which is how [#2473](https://github.com/Selftend/selftend/issues/2473)'s `blocked_by` edge rotted twice.
...
- **Resource shrinking, and the images it must not strip** ([#2211](https://github.com/Selftend/selftend/issues/2211)). Every image the app `require()`s — the help illustrations, the onboarding and info modal illustrations, the Google mark on the auth buttons, the app mark — becomes an Android `res/drawable-*` entry that no Java or Kotlin code references: React Native resolves it at runtime by a name computed from the asset path (`getAndroidResourceIdentifier` in `@react-native/assets-registry`, `images_help_cbt_program` for `assets/images/help/cbt_program.png`), and a name that lives only in the Hermes bundle is not a root the resource shrinker can see. What keeps them is a file the bundler writes, not a rule in this repo: `@expo/cli`'s `persistMetroAssets` (`createKeepFileAsync`, run for the Android platform when the release build embeds the bundle) writes `res/raw/keep.xml` with a `tools:keep="@drawable/…,@raw/…"` entry for **every** asset it copies, and the Android Gradle plugin records `tools:keep` entries as explicit shrinker roots. So `shrinkResources` relies on Expo's generated keep file, deliberately, and no hand-written keep rule is maintained beside it — a `-keep` rule cannot express a resource, and a hand-kept list would drift the day an image is added. `test/android-app-optimisation.test.ts` pins the reliance: it scans every image `require()` in `app/` and `src/`, asserts each file exists, and asserts the installed `@expo/cli` still writes the keep file for every asset — so an Expo upgrade that drops or narrows the generator fails the build rather than the next release. ⚠️ What no test can pin is the artefact: on the next `bundleRelease`, confirm `res/drawable-*/assets_images_help_*.png` still carry real bytes — AGP replaces a stripped file resource with a stub (`DummyContent`, a tiny PNG) rather than deleting the table entry, so "the id resolves" is not evidence. The 2026-09-03 proof build counted only DEX.

**What a machine can verify, and what it cannot.** The proof this shipped with was a local `npx expo prebuild --platform android --clean` followed by `gradlew :app:bundleRelease` with `SENTRY_DISABLE_AUTO_UPLOAD=true`, measured against the 0.17.0 AAB kept as a `release.yml` run artifact:

| Release AAB              | DEX files | DEX bytes (uncompressed) | `BUNDLE-METADATA/com.android.tools.build.obfuscation/proguard.map` |
| ------------------------ | --------- | ------------------------ | ------------------------------------------------------------------ |
| 0.17.0 (shipped)         | 6         | 54,128,320               | absent                                                             |
| this config (2026-09-03) | 3         | 20,227,356               | present, 71 MB, 733,366 lines                                      |

`mapping.txt` kept `org.vasilyoshev.selftend.widget.SelftendCard` and `com.reactnativeandroidwidget.RNWidgetProvider` under their own names. Two things the proof build taught, both now in `app.config.ts`: on Windows the build must run from a short path (ninja could not create its object directory under `.claude/worktrees/...`, a 260-character limit), and the first `minifyReleaseWithR8` died with `OutOfMemoryError: Metaspace` on Expo's template `-XX:MaxMetaspaceSize=512m` — on a daemon reused across two builds, so a fresh CI daemon may fit, but the ceiling is raised to 1 GB (heap 4 GB) rather than measured to the edge, because a failed release build costs a version code.

It does not show a launcher widget rendering, a push arriving, or Sentry symbolicating a native frame; those need a device and a `preview` build with a Sentry token in the shell. **Never a `production` build to test this:** that profile has `autoIncrement`, which consumes a version code per _failed_ build. The Play card itself is only re-read after the next production release, since it is computed per upload.

#### Play's three later suggestions, and what was done with each ([#2335](https://github.com/Selftend/selftend/issues/2335))

With R8 live, Play Console replaced the Feb-2027 threshold warning with a lower-severity performance card — **no `Bad behaviour` chip, no deadline, no publishing consequence**. It named three things. They did not have the same answer, and the reasoning matters more than the outcome.

- **Optimised resource shrinking — taken.** ☠️ **The property is `android.r8.optimizedResourceShrinking`**, an AGP 8.12 opt-in (React Native 0.86 pins exactly 8.12.0) that AGP 9.0 turns on by default; this is an early opt-in to a coming default, not a divergence. The card's write-up named an `android.` + `enableOptimizedResourceShrinking` spelling, **which is not a property that exists** — and Gradle drops an unknown property in silence, so that spelling builds green, passes the #2211 audit and shrinks nothing. `test/android-app-optimisation.test.ts` pins the name for that reason alone. It folds resources into R8's single reference graph rather than a separate AAPT2 pass, dropping the unconditional keeps AAPT2 used to generate, so it is **strictly more aggressive** and the risk lands exactly on the images React Native resolves by runtime name. What holds is that `tools:keep` remains the documented mechanism and Expo still generates the keep file. **Measured, not assumed** (local `bundleRelease`, 2026-09-16, both builds from the same tree):

  |                                              | Baseline            | Optimised shrinking         |
  | -------------------------------------------- | ------------------- | --------------------------- |
  | AAB                                          | 95,679,925 B        | **94,996,504 B** (−683,421) |
  | DEX (3 files, uncompressed)                  | 20,241,208 B        | **20,177,600 B** (−63,608)  |
  | `base/res/` + `base/dex/` entries            | 1,389               | **965** (−424)              |
  | The 42 `require()`d images and 19 sound beds | present, real bytes | **byte-for-byte identical** |

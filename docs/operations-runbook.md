...
- **Monthly, on the 9th - the Ahrefs Site Audit of `selftend.org`** (project 10345616; Execute JavaScript on; URL sources: the website plus the auto-detected sitemap, no custom list; the two `(auth)` URLs excluded from the crawl by pattern; four issues turned off for the project - the two description-length warnings and the two AI-bot notices - because none of them can move when the site does). It reads whether the site is still readable to a crawler: every page on the index list crawled with its own title, description and canonical, no duplicate group, the Health Score at **100** over 16 crawled URLs (8 pages, `sitemap.xml`, the `www` 301, 3 resources, 3 `robots.txt` fetches) - 88 until the two privacy-page links ship ([search-operations.md](search-operations.md) § 9). **Expected rows, never a regression:** `3XX redirect` ×1 (the `www` seed answering § 6.1's redirect) and `only one dofollow incoming internal link` ×7 (×5, plus `Orphan page` ×2, until those links ship). **Noise, not a regression:** `Slow page` on a single crawl, and `Pages added to sitemaps` on the crawl after a sitemap change. **An audit regression:** any Error on an index-list URL, any issue type not named here, a change in a listed page's status, canonical or description, or `Slow page` on the same page two audits running. A regression here is a build defect and goes to a GitHub issue, never to the marketing plan - the plan reads Search Console once a year, on 2026-12-10, coverage and one standings reading beside it, and nothing else ([marketing-plan.md](marketing-plan.md) § 7; decided on [#2289](https://github.com/Selftend/selftend/issues/2289) and [#2425](https://github.com/Selftend/selftend/issues/2425)). Google's emails on indexing issues, manual actions and security are default-on and are the coverage alarm between sittings; do not unsubscribe. The `(auth)` routes' 404 is verified by [deployment.md](deployment.md)'s post-deploy check, not by the audit. Never re-enable a turned-off issue or "fix" a by-design 404 to move the score; the policy and its reasons are on [#2424](https://github.com/Selftend/selftend/issues/2424) and in [search-operations.md](search-operations.md) § 3-5, and the spec the pages are read against is [indexability.md](indexability.md). If the turned-off list or the exclusion is missing after the Ahrefs plan change, re-apply it from that ticket before reading the crawl. ⚠️ The audit runs on whichever Ahrefs plan is active; it needs no paid tier. Starter is cancelled and the workspace is free Webmaster Tools from 2026-10-10 ([costs.md](costs.md)).

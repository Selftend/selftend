...
| Managed robots.txt         | AI Crawl Control → Signals → "Managed robots.txt" (it sat under Security → Bots when [#2288](https://github.com/Selftend/selftend/issues/2288) was decided; location checked 2026-09-10; <https://developers.cloudflare.com/bots/additional-configurations/managed-robots-txt/>) | **on**. Cloudflare prepends its content-signals block to `public/robots.txt` and never replaces it, so the repo's `User-agent: *`, empty `Disallow:` and `Sitemap:` lines follow it                                                                                                                        | `curl -s https://selftend.org/robots.txt` reads the managed block first, then the repo's three lines, and **no HTML** (before the export shipped `robots.txt`, the whole app shell was appended instead). Toggle confirmed on 2026-09-10; the combined file **verified 2026-09-10** once v0.19.0 shipped `public/robots.txt`. ⚠️ `https://www.selftend.org/robots.txt` and `http://selftend.org/robots.txt` answer **200 with only the managed block** - no `Sitemap:` line, none of the repo's lines - because the managed file answers at the edge before the redirect rule and Always Use HTTPS run. Harmless: every page on those origins is a 301, and Google reads the sitemap from the apex file and Search Console. Read 2026-09-15 ([#2420](https://github.com/Selftend/selftend/issues/2420)) |

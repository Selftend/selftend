...
- **Production** (`selftend.org`, Worker `selftend`; `www.selftend.org` is bound to the same Worker only so the zone-level rule in "Cloudflare zone settings" below can answer it with a 301 to the apex): deploys on a **published Release** (`release.yml`, after the prod DB migration gate). Manual fallback: the `Web production deploy` (`web-deploy.yml`) workflow in the Actions tab.
- **Staging** (`selftend-staging.vasil-yoshev.workers.dev`, Worker `selftend-staging`): deploys on **push to `dev`** (`staging.yml`, after the staging DB migration gate). Staging is served `noindex`. ⚠️ **Not on the `selftend.org` zone** — the `staging.selftend.org` custom domain was removed on 2026-09-23 ([#2319](https://github.com/Selftend/selftend/issues/2319)) because Cloudflare's analytics dataset has no host dimension on any plan, so staging's traffic was being counted as production's. The zone settings in the table below therefore **do not apply to staging**; `public/_headers` still does.
...
Three settings on the `selftend.org` zone live in the Cloudflare dashboard and in no file in this repo, so this table is their only record in the repo ([indexability.md](indexability.md) § 6.1, § 6.2, § 6.5 and § 6.8; decided on [#2288](https://github.com/Selftend/selftend/issues/2288), applied and verified on [#2297](https://github.com/Selftend/selftend/issues/2297), 2026-09-10 to 2026-09-11). Together with the Worker's real 404s they make `https://selftend.org` the only serving origin: `www` and plain `http://` each answer a 301 instead of a second copy of the site. Each row carries the `curl` that verifies it from outside and the date it was last verified; re-run the `curl` and update the date whenever the zone is touched. The control-tower inventory carries the same three settings (control-tower [#132](https://github.com/vasilyoshev/control-tower/issues/132)), so a correction here is owed there too.

☠️ **A row here can stop being true without anything going red**, which is the whole reason it carries a date and a `curl` rather than a description. It has happened once: the Bot Preference Sync row asserted a working content-signals prepend for twelve days after the prepend had stopped, while the dashboard toggle went on reporting it as on, managed and declared ([#2702](https://github.com/Selftend/selftend/issues/2702) - since resolved by turning the setting off, so the two now agree). ⚠️ So **the toggle is not the verification** - run the `curl` and write the date, even when the dashboard agrees with you.

| Setting                                        | Where in the dashboard                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Verified from outside                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `www` → apex redirect rule                     | Rules → Overview → the rule named "www to apex" (created via Create rule → Redirect Rule, "Custom filter expression")                                                                                                                                                                                                                                                                                                                                                         | expression `http.host eq "www.selftend.org" and not starts_with(http.request.uri.path, "/.well-known/")`; redirect type **Dynamic**, expression `concat("https://selftend.org", http.request.uri)`; status **301**; leave "Preserve query string" off (the default) - the target already carries the query                                                                                                                                                                                                                                                     | `curl -sI https://www.selftend.org/faq` → `301` with `location: https://selftend.org/faq`; `curl -sI "https://www.selftend.org/faq?x=1"` keeps `?x=1`; `curl -sI https://www.selftend.org/.well-known/apple-app-site-association` → `200`. **Verified 2026-09-22** - all three pass; `http://www` 301s straight to the HTTPS apex in one hop                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Always Use HTTPS                               | SSL/TLS → Edge Certificates → Always Use HTTPS                                                                                                                                                                                                                                                                                                                                                                                                                                | **on** (zone-wide: apex and `www`). ⚠️ **`staging` is no longer on this zone** and so is not covered — it moved to `selftend-staging.vasil-yoshev.workers.dev` on 2026-09-23 ([#2319](https://github.com/Selftend/selftend/issues/2319)). No HSTS preload submission - the `preload` token in `public/_headers` stays as it is ("HSTS preload" below)                                                                                                                                                                                                          | `curl -sI http://selftend.org/` → `301` with `location: https://selftend.org/`; `curl -sI http://www.selftend.org/` → `301` with `location: https://selftend.org/`. **Verified 2026-09-23**, re-run after staging left the zone. ⚠️ The old third check (`curl -sI http://staging.selftend.org/`) is **gone on purpose, not lapsed** — that host is now NXDOMAIN, and a check against it would prove nothing about this zone. ⭐ The consequence, written down so it is not rediscovered as a surprise: `curl -sI http://selftend-staging.vasil-yoshev.workers.dev/` answers **200 in cleartext**, because this setting does not reach a `workers.dev` host. Staging's own `Strict-Transport-Security` header (from `public/_headers`, verified present 2026-09-23) upgrades every request after the first, and `x-robots-tag: noindex` still serves. Accepted for a `noindex` staging surface with no production data                                                                                                                                                                                                                                                                  |
| Bot Preference Sync (was "Managed robots.txt") | AI Crawl Control → **Signals** → "Enable Bot Preference Sync" (`/<account>/selftend.org/ai/signals`). ⚠️ **Renamed and moved twice.** It sat under Security → Bots when [#2288](https://github.com/Selftend/selftend/issues/2288) was decided, then at `…/ai-crawl-control/signals` - a path that now renders Cloudflare's own "Page not found". Location and name checked 2026-09-22; <https://developers.cloudflare.com/bots/additional-configurations/managed-robots-txt/> | **off, by decision** ([#2702](https://github.com/Selftend/selftend/issues/2702), turned off 2026-09-23). Selftend declares **no AI-crawl preference**. It was never argued on its merits - it arrived with the [#2288](https://github.com/Selftend/selftend/issues/2288) zone setup - and it had demonstrably not worked since at least 2026-09-15 without any consequence anyone could name. ⭐ The point of turning it off rather than repairing it: the dashboard and the wire now **agree**, which is the only state this row cannot silently drift out of | `curl -s https://selftend.org/robots.txt` returns **67 bytes, byte-for-byte `public/robots.txt`** - `User-agent: *`, empty `Disallow:`, the `Sitemap:` line, no managed block before or after, and no HTML. **Verified 2026-09-22** ([#2702](https://github.com/Selftend/selftend/issues/2702)), and re-run against `GPTBot/1.0`, `ClaudeBot/1.0` and `CCBot/2.0` as well as a browser UA - all four identical, so it was **not** crawler-conditional. ⭐ **Re-verified 2026-09-23, immediately after the toggle was turned off: still 67 bytes, still byte-for-byte identical to `public/robots.txt`.** That the file did not move is the confirmation, not a null result - it is what proves the prepend had genuinely stopped rather than being switched off mid-flight. ⚠️ The edge no longer intercepts this path at all: `https://www.selftend.org/robots.txt` and `http://selftend.org/robots.txt` answer an ordinary **301** to the apex, where until 2026-09-15 they answered 200 with only the managed block. ✅ **The site is unaffected** - the file crawlers read is intact and correct, and what is absent is an AI-crawl preference declaration, not an indexing control |

The `/.well-known/` carve-out in the redirect rule exists because Apple and Google both refuse an association file that answers with a redirect, and iOS builds up to v0.19.0 claim `applinks:www.selftend.org`. The claim left `app.config.ts` on [#2298](https://github.com/Selftend/selftend/issues/2298); the carve-out is retired **once a build without the claim is the current App Store version** - the one-time owner step in [launch/app-links-runbook.md](launch/app-links-runbook.md) § _Retiring the `www` carve-out_, which ends by writing the date into the row above and deleting this paragraph ([indexability.md](indexability.md) § 6.3). Nothing else on `www` needs to answer 200.
...
Verify against a deployed Worker with `curl -I`:

```bash
curl -sI https://selftend-staging.vasil-yoshev.workers.dev/_expo/static/js/web/index-<hash>.js
```

If this machine's DNS resolver lags behind a freshly-changed record, pin the IP with `--resolve <host>:443:<cloudflare-ip>`. That was routine while staging sat on the `selftend.org` zone and its records were new; the `workers.dev` hostname is long-lived, so it is rarely needed now.
...
The HSTS header in `public/_headers` carries the `preload` token, and the domain is **not** submitted to <https://hstspreload.org> - decided on [#2288](https://github.com/Selftend/selftend/issues/2288) ([indexability.md](indexability.md) § 6.2): a preload-list entry is irreversible in practice and nothing here needs it. The token stays as it is because removing it changes nothing for visitors and would touch a pinned header. What forces HTTPS is the zone's Always Use HTTPS setting ("Cloudflare zone settings" above); the header then keeps a browser that has visited once on HTTPS for a year.

## Public Routes To Verify

These routes must be reachable without signing in. The ten on the index list answer **200** with their own file, in the list's own order (the site footer's):

- `/` (the landing page)
- `/crisis`
- `/meditation` (the meditation framework — the whole page must read with no account)
- `/habits` (the ten core ideas — all ten bodies must read with no account, and no card is a link)
- `/faq`
- `/privacy`
- `/terms`
- `/cookies`
...
- open `/privacy`, `/terms`, `/crisis`, `/account-deletion`, and the two explainer pages `/meditation` and `/habits` (each must read in full with no account)

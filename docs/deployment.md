...
| Managed robots.txt         | AI Crawl Control → Signals → "Managed robots.txt" (the toggle moved there from Security → Bots) | **on**. Cloudflare prepends its content-signals block to `public/robots.txt` and never replaces it, so the repo's `User-agent: *`, empty `Disallow:` and `Sitemap:` lines follow it                                                                                                                  | `curl -s https://selftend.org/robots.txt` reads the managed block first, then the repo's three lines, and **no HTML** (before the export shipped `robots.txt`, the whole app shell was appended instead). Toggle confirmed on 2026-09-10; the combined file is checked after the release that ships `public/robots.txt` |

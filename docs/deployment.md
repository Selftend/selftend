...
☠️ **A row here can stop being true without anything going red**, which is the whole reason it carries a date and a `curl` rather than a description. It has happened once: the Bot Preference Sync row asserted a working content-signals prepend for twelve days after the prepend had stopped, and the dashboard toggle still says it is on ([#2702](https://github.com/Selftend/selftend/issues/2702)). ⚠️ So **the toggle is not the verification** - run the `curl` and write the date, even when the dashboard agrees with you.
...
| Bot Preference Sync (was "Managed robots.txt") | AI Crawl Control → **Signals** → "Enable Bot Preference Sync" (`/<account>/selftend.org/ai/signals`). ⚠️ **Renamed and moved twice.** It sat under Security → Bots when [#2288](https://github.com/Selftend/selftend/issues/2288) was decided, then at `…/ai-crawl-control/signals` - a path that now renders Cloudflare's own "Page not found". Location and name checked 2026-09-22; <https://developers.cloudflare.com/bots/additional-configurations/managed-robots-txt/> | **on, and not taking effect.** The toggle is enabled and Cloudflare's own _Robots.txt availability_ table reports every hostname as **Status: Cloudflare Managed / Content Signals: Declared** - but nothing is prepended to what is actually served. ☠️ Treat the dashboard's claim as unverified: this row records the wire, not the toggle | `curl -s https://selftend.org/robots.txt` returns **67 bytes, byte-for-byte `public/robots.txt`** - `User-agent: *`, empty `Disallow:`, the `Sitemap:` line, no managed block before or after, and no HTML. **Verified 2026-09-22** ([#2702](https://github.com/Selftend/selftend/issues/2702)), and re-run against `GPTBot/1.0`, `ClaudeBot/1.0` and `CCBot/2.0` as well as a browser UA - all four identical, so it is **not** crawler-conditional. ⚠️ The edge no longer intercepts this path at all: `https://www.selftend.org/robots.txt` and `http://selftend.org/robots.txt` now answer an ordinary **301** to the apex, where until 2026-09-15 they answered 200 with only the managed block. ✅ **The site is unaffected** - the file crawlers read is intact and correct, and what is missing is an AI-crawl preference declaration, not an indexing control. Whether it should be declared at all is open on [#2702](https://github.com/Selftend/selftend/issues/2702) |

...
- [brand-result.md](brand-result.md) - the brand-result spec (decided; being built - the footer and the two structured-data properties have landed, the explainer pages are next): what a search for Selftend can truthfully show - explainer pages from content the app already holds, one footer linking every public page, two more structured-data properties, and the honest ceiling on sitelinks and knowledge panels
...
- [reminders.md](reminders.md) - the reminders spec (decided, not built): one general reminder, suggested by placement alone on the Reminders screen and the settings card, and every per-tool and routine reminder manual-only; the data model, the copy in both locales, and the numbered build list
...
- [marketing-plan.md](marketing-plan.md) - the nine marketing squares as decisions, the number the plan answers to, and what the book demands that Selftend refuses

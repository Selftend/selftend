...
- [reminders.md](reminders.md) - the reminders spec (built, bar the release and the hold-out lift): one general reminder, suggested by placement alone on the Reminders screen and the settings card, and every per-tool and routine reminder manual-only; the data model, the copy in both locales, and the numbered build list

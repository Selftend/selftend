...
The gate renders below the layout's session branch, so an auth user exists by
the time the verdict is known — on **all four** entry paths, not the three §3
predicted ([#1919](https://github.com/Selftend/selftend/issues/1919)). §3
describes this as an OAuth-specific deletion; it is not, and the silent guest is
what makes it the common case. It goes through

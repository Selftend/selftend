...
>
> ⚠️ **The reason the old sentence gave has not been answered on iOS — it has been overridden there.** `docs/positioning.md` clause 1 still says no surface may present the tools without the method, and an iOS production Home now does exactly that. That conflict is live and recorded beside clause 1 itself, in that document's § 1 "The canvas"; it is not resolved here, and this note must not be read as resolving it. On Android and web the old reason still holds and the section still renders. The catalogue constant is still the eleven — the gate filters at the render sites, so "catalogue order" keeps one referent.

...
- **Everything else** — ACT — has no captured offset and uses the **viewer's current local day**. ACT is deliberately out of scope until it grows a history or calendar surface (#330 owner decision); every ACT surface is a same-day list today. Routines deliberately keep a viewer-local day axis whatever their steps use: a routine has no dated record to freeze, and its job is "today, where you are standing". Routine status resets at local midnight.
- The **CBT programme checklist** is the one remaining viewer-local consumer of tools that now capture an offset: its `thoughtRecordDaily`, `activityDaily` and `calmingDaily` legs exist twice, once client-side and once in the `program_widget_task_status` RPC, and both copies graduate together per module in #425. Moving one side alone would make the home widget and the programme screen disagree.

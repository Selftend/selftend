...
**mental health tools**:
What Selftend is, in the words it says so in — the category noun, a bare plural with no article and no qualifier, spelled the same way everywhere ([#2004](https://github.com/Selftend/selftend/issues/2004)). _A set of_ may stand in front of it in the frame sentence and nowhere else. The method — a **CBT programme**, _cognitive behavioural therapy_ spelled out on first use on a surface, then **CBT** — is **not** inside the noun, and since 2026-09-18 it is not in the frame sentence either; it is named on the module's own surfaces, and it is run by the person using it with no practitioner involved. See [docs/positioning.md](docs/positioning.md) § 3 for the record of that change. The noun alone is a flat inventory of utilities. (The previous noun, _CBT self-help app_, carried the method inside itself — [#1814](https://github.com/Selftend/selftend/issues/1814); that move was surrendered on #2004.)
...
**programme**:
The noun of a module's own staged progression — authored, phased, and graduated from once: the CBT programme (five phases), the ACT programme (four), and the DBT programme (four, decided in [docs/modules/dbt-mckay-skills-workbook.md](docs/modules/dbt-mckay-skills-workbook.md) §4 and built on map [#1980](https://github.com/Selftend/selftend/issues/1980)). The word belongs to every module that has one ([#1991](https://github.com/Selftend/selftend/issues/1991)). ☠️ It is **not** the product's category any more: [#1814](https://github.com/Selftend/selftend/issues/1814) kept the word for the component and moved the category to the entry above, and [#2004](https://github.com/Selftend/selftend/issues/2004) kept it there when the category moved again.
...
**Favourite**:
A tool or a module the person has starred to keep on Home — one of the eight everyday tools or one of the three modules, and nothing finer-grained than that ([#1885](https://github.com/Selftend/selftend/issues/1885)). Home lists the favourites first, then the complete catalogue of eleven, through the same card; a favourited item therefore appears twice, plainly. The star is a toggle with an immediately visible consequence, so there is no cap and no "full" state. Favourites is the catalogue **filtered**, never sorted — nothing stores an order — and the modules section beneath it renders on every build that shows modules at all.

> ☠️ **That section used to render _unconditionally_, and the reason it no longer does is an owner instruction, not a discovery.** The sentence here read: "the modules section beneath it renders unconditionally, because a Home that shows the tools without the method is the inventory `docs/positioning.md` forbids". The three modules are now behind a build gate (`modulesAreVisible`, `src/lib/module-visibility.ts`), and **the gate is iOS-only**: Android and web show all three in every build, labelled **beta**; iOS shows them only in a development build. So an **iOS production** Home shows the eight tools and no method, and no other platform does.
>
> ⚠️ **The reason the old sentence gave has not been answered on iOS — it has been overridden there.** An iOS production Home presents the eight tools with no method behind them. `docs/positioning.md` clause 1 once forbade exactly that; the clause was **retired on 2026-09-18** when the frame sentence lost the beat that satisfied it, so there is no longer a conflict to resolve — see that document's § 3 for the record. On Android and web the modules still render in this section. The catalogue constant is still the eleven — the gate filters at the render sites, so "catalogue order" keeps one referent.

...
\echo '    an exact fact about EVERY MEMBER of that cohort. Deliberate, not an omission: an arm at nought or'
\echo '    a hundred per cent is the strongest reading this instrument can produce, and banding it away would'
\echo '    blunt the finding to guard against a reader who already holds a database credential. See'

...
⚠️ **This mirror lags the governing document by one decided edit, and it must keep lagging until the Console visit that applies it.** The short description below is the 28-character short form as saved on 2026-09-05 (evening) — in review, see the date line. The first paragraph still carries the **dash** after the noun (_tools — everyday tools_) where `docs/positioning.md` decided a **colon** (_tools: everyday tools_, [#2007](https://github.com/Selftend/selftend/issues/2007)): the agent's full-description edit was refused by the auto-mode classifier on 2026-09-05, so that one-character edit is an owner Console visit. ☠️ **The bundling this paragraph counted on did not happen.** On 2026-09-06 the feature graphic went up and was submitted alone ([#2022](https://github.com/Selftend/selftend/issues/2022)); the classifier refused the full-description write a **second** time, on a fresh visit, in a different tool. Two refusals on the same field is a pattern rather than an accident, so stop planning to ride that mark along with something else: it costs its own review, and the next agent that tries will be the third. Do not "fix" this file ahead of the store: it mirrors what the Console holds, and the diff on the day it moves is the record.
...
☠️ **A second defect rode in the same image and no copy edit reached it** ([#2041](https://github.com/Selftend/selftend/issues/2041)). The three phone mockups were pre-v0.5.0 captures that legibly read **"Cognitive Behavioral Therapy"**, **"Your CBT program"** and **"Start program"** — the American spellings `verify` bans in app copy ([#1627](https://github.com/Selftend/selftend/issues/1627), [#1651](https://github.com/Selftend/selftend/issues/1651)), published inside a store asset. Those words exist only as pixels, so the fix was new captures from a current build, not an edit to the HTML.

✅ **Retaken 2026-09-06 and the PNG re-rendered.** ☠️ The premise that this needed a device and therefore a human was wrong twice over: the mockups are **browser** captures at 390×844, and `staging.selftend.org` was already serving the current British spellings while **production still lagged** — so the capture source mattered more than the capture device. All three were replaced rather than just the offending one, because the middle mockup's Dashboard screen no longer exists in that shape and a lead image mixing two app generations is a worse asset than the one it replaces. ⚠️ **The repository asset and the Console asset are now one review apart**: the graphic submitted earlier the same day carries the OLD mockups, so putting this one live is a further submission — bundle it with the #201 screenshot slots rather than spending a review on it alone, and do not stack it on the in-flight one. Recipe and the eye-check it needs: `docs/launch/play-listing/README.md`.

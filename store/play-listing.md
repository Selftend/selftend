...
☠️ **One contradiction is live outside the text: the feature graphic.** Seen in the Console's review-assets step on 2026-09-05, the feature graphic's artwork still carries the retired compound from the first table row above ("Calm, guided …"). It is an image, so no gate — not `test/positioning-copy.test.ts`, not this file — can see it, and `docs/launch/play-listing/feature-graphic.png` in the repo is the same artwork. Replacing it is a Console asset edit (one more review) and needs a new graphic first; tracked on [#1999](https://github.com/Selftend/selftend/issues/1999).

...
✅ **Retaken 2026-09-06 and the PNG re-rendered**, all three mockups rather than only the offending one. ⚠️ **The repository asset and the Console asset are now one review apart**: the graphic submitted earlier the same day carries the OLD mockups, so putting this one live is a further submission — bundle it with the #201 screenshot slots rather than spending a review on it alone, and do not stack it on the in-flight one. ☠️ Which build you capture from decides whether the retake works at all, and it is not the obvious one; that, the recipe, and the eye-check no test can do are in `docs/launch/play-listing/README.md` rather than repeated here.

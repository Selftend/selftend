...
⚠️ **`apple-info.json` holds two fields, not four.** [#1606](https://github.com/Selftend/selftend/issues/1606) specified `subtitle`, `description`, `keywords` and `promoText`; only `subtitle` and `promoText` are committed. `keywords` is a hidden field that **cannot be read from outside App Store Connect at all**, and it is a welcome addition the moment someone reads it from the live record.

...
- Per-PR review gates live in `.github/pull_request_template.md`. **Opening a PR means filling that template in** - the summary, the review context and the product-guardrail checklist - rather than writing a free-form body. Nothing enforces this and nothing should: a checklist nobody is made to tick is a prompt, and a gate that turns honest blanks into unread ticks is worse than the gap it closes (decided on [#2672](https://github.com/Selftend/selftend/issues/2672)). ⚠️ So **a box left blank on purpose is a better answer than one ticked without reading**, and `N/A` with a reason beats a bare tick. The rule exists because nineteen PRs in six days carried no template at all - not a partial one - and the audit that followed had to read eleven diffs by hand to establish that nothing had slipped through.
...
- **Loading surfaces:** a pending state that claims a fact it does not have (a dash, "Nothing yet", a placeholder figure), or one that gives back the space it will occupy so everything below it jumps when the data lands — [ADR-0009](docs/adr/0009-reserve-the-space-or-draw-nothing.md). Both clauses, not either one. A shift that moves a control under a finger is an input-integrity defect, not a cosmetic one; reserve with the real content at `opacity-0` (`ReservedSpace`), and fill visibly only where arrival is guaranteed. Where the final height genuinely is not knowable — nothing already in hand decides it — the content moves to the **end of its scroll column** instead (edge 5), so there is nothing below it to push; `manage-emotions-modal.tsx` is that case, and a reservation there would be the guessed height the ADR rejects. So a loading surface carrying no `ReservedSpace` is not by itself a finding.

...
- **Loading surfaces:** a pending state that claims a fact it does not have (a dash, "Nothing yet", a placeholder figure), or one that gives back the space it will occupy so everything below it jumps when the data lands — [ADR-0009](docs/adr/0009-reserve-the-space-or-draw-nothing.md). Both clauses, not either one. A shift that moves a control under a finger is an input-integrity defect, not a cosmetic one; reserve with the real content at `opacity-0` (`ReservedSpace`), and fill visibly only where arrival is guaranteed.

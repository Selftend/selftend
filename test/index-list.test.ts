...
describe("the index list ↔ the route tree", () => {
  ...
  // explainers, then policies. The footer pin in
  // `src/components/app/site-footer.test.tsx` compares ordered arrays against
  // this list, so the two sequences cannot drift.
  //
  // ☠️ `/meditation` is INSERTED, not appended (#2469): it is the first
  // explainer, and the explainer row renders above the policies. Appending it
  // passes every assertion in this file and fails the footer's ordered pin.
  it("holds the nine routes the spec's § 3 table names", () => {

...
test("a breadcrumb returns to its ancestor instead of stacking a second copy", async ({ page }) => {
  ...
  // card carries no `accessibilityLabel` (#1955), so its accessible name is every child
  // it renders - the mark, the name and the subtitle - and no longer the name alone.

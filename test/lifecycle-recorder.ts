...
/**
 * Mount and unmount of whatever is under record, in order.
 *
 ...
 * and it is the ONLY reason the indirection exists. `babel-preset-expo` runs with
 * `react-compiler` and `jsxImportSource: "nativewind"`, both of which rewrite a
 * component to reference a module-scope import (`_ReactNativeCSSInterop`). A
 * component declared INSIDE a `jest.mock()` factory therefore trips jest's
 * out-of-scope-variable guard, and the suite dies before a single test runs.
 * Declared here, the factory only has to `require` it.
 *
 * `Props` is generic because a pass-through wrapper has to forward whatever the
 * wrapped component takes - not because a second caller is anticipated. There is
 * one (`app-toast.test.tsx`); inline it back if that stops being true.
 */

...
## 4. Pre-Public Launch Cleanup Checklist

### 4.1 Content & Copy Review

- [x] **README.md cleanup** - Trimmed to core content: project description, quick start, documentation links, license, contributing (183 lines → 42 lines). Removed: "Status" section (GitHub Issues track progress), "Current implementation" bloat, "Product direction", "Repo map", "Documentation expectation", "Reference repositories"
- [ ] **Policy content finalization** - Replace placeholder organization name with confirmed legal entity
- [ ] **Email address verification** - Confirm domain-based emails are configured
- [ ] **Legal review** - Complete human/legal review of privacy policy, terms, and crisis guidance
- [ ] **Crisis guidance jurisdiction review** - Verify jurisdiction-specific resources are accurate

### 4.2 Documentation Cleanup

- [ ] **Audit docs/** - Review for outdated or redundant content
- [ ] **Update ROADMAP references** - Ensure completion status references are current
- [ ] **Remove internal notes** - Clean up working-out text and temporary notes
- [ ] **Verify URLs** - Ensure all documentation links point to working targets

### 4.3 Code Cleanup

- [ ] **Remove TODOs and FIXMEs** - Only found in `expo-env.d.ts` (autogenerated, safe to ignore)
- [ ] **Remove console statements** - Clean up debug logging before public release
- [ ] **Audit JSDoc comments** - Remove generic or verbose comments where code is self-evident
- [ ] **Review unused imports** - Clean up dead code and unused dependencies

### 4.4 Translation & i18n Review

- [x] **Verify translation keys** - Translation keys verified clean: meaningful naming, no AI-bloat, proper coverage for supported languages (en, bg)
- [ ] **Check translation coverage** - Verify all user-facing strings have translations for supported languages

### 4.5 Legal & Compliance

- [ ] **THIRD_PARTY_NOTICES.md review** - Verify all third-party dependencies and their licenses are correctly attributed
- [ ] **License headers** - Confirm source files have appropriate license headers where needed

### 4.6 Technical Verification

- [ ] **Environment variable documentation** - Verify all `.env.example` values are documented
- [ ] **CI/CD verification** - Confirm GitHub Actions workflows are functional
- [ ] **Domain/DNS check** - Verify production domain configuration if applicable
- [ ] **Supabase redirects** - Verify production redirect URLs are configured correctly

---

_This checklist was added on 2026-05-11 as part of pre-public-launch preparation. Progress can be tracked and items can be completed incrementally._

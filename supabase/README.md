...
> - **The routine reminder UI is unreviewable from the seed.** Both seeded routines have reminders off deliberately (notifications are explicit and quiet by default), so the reminder time picker and its permission prompt only render if you turn one on by hand.
> - **Routine cadence covers one variant of four.** Both routines are `daily`, because scheduled-ness gates Home's routines widget, the floating progress button and the continue sheet — a `weekdays`, `custom` or `on-demand` routine would make those three surfaces depend on which weekday the seed happened to run. Reviewing the other three cadences means editing a routine.
> - **Home has no widget layout at all on a freshly reset stack.** `widget_preferences` is written only by onboarding's concern resolution and by the Add-Widget flow behind `/arrange`, and demo is seeded with onboarding already complete — so its Home renders no tool rows until you add some, the routines row included. Add them from **Home → Arrange**; they last until the next reset. A long-lived local database hides this, because the rows on it were added by hand. Tracked in [#1352](https://github.com/Selftend/selftend/issues/1352).

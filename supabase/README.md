...
> **Decided, but not yet seeded.** Both were charted on [#1511](https://github.com/Selftend/selftend/issues/1511) and every question is settled — the implementable spec is on each issue, so neither needs re-deciding.
>
> - **Home carries no widget layout on a freshly reset stack.** `widget_preferences` is written only by onboarding's concern resolution and by the Add-Widget flow behind `/arrange`, and demo and bob are both seeded with onboarding already complete — so their Home renders no tool rows until you add some, the routines row included. Add them from **Home → Arrange**; they last until the next reset. A long-lived local database hides this, because the rows on it were added by hand. Decided: demo carries 14 ids, bob the 4 that onboarding emits for `anxious-thoughts` plus CBT, and alice stays at zero. [#1352](https://github.com/Selftend/selftend/issues/1352)

...
> **Decided, but not yet seeded.** Charted on [#1511](https://github.com/Selftend/selftend/issues/1511) and every question is settled — the implementable spec is on the issue, so it does not need re-deciding.
>
> - **Routine cadence covers one of the three seedable variants, and no routine carries a reminder.** Both seeded routines are `daily` with reminders off. (`custom` is the fourth and is permanently excluded — see the note above — so three is the ceiling, not four.) ⚠️ That is **not** for the reason the seed script used to give: `isScheduledOn` is evaluated at render against `new Date()`, not at seed time, so a non-daily cadence costs nothing in reproducibility — only the day you happen to look changes, which the seeded window has always depended on. Decided: four routines — one `weekdays`, two `daily`, one `on-demand` — carrying exactly one reminder, at 08:00 `Europe/Sofia`. The same change corrects demo's reminder consent: `seed.sql` writes `true`, and `scripts/seed-demo-data.mjs` then overwrites it with a `false` that reads as _declined_, suppressing the one-time contextual reminder prompt on every tool and leaving an armed CBT reminder the server can never deliver. [#1271](https://github.com/Selftend/selftend/issues/1271)

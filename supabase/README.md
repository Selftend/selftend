...
- `guest-dormancy-cleanup.integration.test.ts` - `cleanup_dormant_guest_accounts(batch_size)`: access control, the greatest-of-three dormancy gate (including an old guest with a recent session refresh surviving), batch limiting, and the pg_cron schedule. Dates its own rows directly in the auth schema via `runSql` (docker exec + psql), since no API writes `auth.users.created_at` or `auth.sessions.refreshed_at`

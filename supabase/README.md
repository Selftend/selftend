...
> **Demo carries `routines-today` even though the widget is `available`, not `default`.** Specs [#37](https://github.com/Selftend/selftend/issues/37)/[#50](https://github.com/Selftend/selftend/issues/50) keep that id out of every auto-seeding surface, and this does not contradict it. The available-not-default decision governs what the **product** offers unasked; it says nothing about what a **fixture's history** contains. `available` means "reachable only by deliberate choice", and demo stands in for a user who made that choice — seeding the id into onboarding's auto-seed path would be the contradiction, seeding it into one fixture's stored layout exercises the status. Without it the seeded routines have no Home surface at all.

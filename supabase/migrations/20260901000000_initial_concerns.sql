-- Immutable concern-at-intake (#1612, decided in #1605).
--
-- `selected_concerns` is last-write-wins: `apply_widget_recommendations`
-- overwrites it, and today-screen.tsx re-runs the wizard from Home through the
-- same RPC. So the concern a person declared when they arrived is recorded
-- NOWHERE, and `scripts/analytics-onboarding.sql` §4a is really reporting
-- *current* concerns.
--
-- That matters because only a RETURNING user re-runs the wizard. Cohorting
-- retention by declared concern is therefore biased in the flattering direction
-- by construction: the users whose value may have been rewritten are precisely
-- the retained ones.
--
-- `initial_concerns` is the same data at first completion, written once. It
-- collects nothing new and needs no consent change (docs/analytics.md Phase 1).
-- `selected_concerns` keeps its mutable behaviour - it drives widget
-- suggestions and must stay live.
--
-- ☠️ NO BACKFILL, decided in #1605. Filling it from today's
-- `selected_concerns` would import exactly the bias it exists to remove; the
-- pre-column population is tens against a read gate of 30 W4-retained users
-- still ahead; and "backfill only never-re-run users" is unavailable because
-- nothing records whether the wizard was re-run. Existing rows stay NULL and
-- are reported as an explicit `unknown` arm.

alter table public.user_preferences
  add column if not exists initial_concerns text[];

comment on column public.user_preferences.initial_concerns is
  'Concerns declared at first onboarding completion. Immutable: written once by apply_widget_recommendations and never updated. NULL means never recorded (pre-#1612 row), which is an explicit "unknown" arm rather than "no concerns".';
...
begin
  ...
  on conflict (user_id) do update set
    ...
    --
    -- ☠️ The second condition is the whole point, and a plain `is null` guard
    -- would defeat it. Only a RETURNING user re-runs this wizard (#1633), so if
    -- a pre-column user's re-run were allowed to fill this in, the rows that got
    -- filled would be exactly the retained ones - re-importing the survivorship
    -- bias the column exists to remove, one row at a time instead of in a
    -- backfill. A user who has completed onboarding before therefore stays NULL
    -- and is reported in the explicit `unknown` arm (#1605).
...
-- reload `initial_concerns` stays invisible to the client until the cache

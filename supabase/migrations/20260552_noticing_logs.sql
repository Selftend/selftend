-- Daily noticing capture: the Think CBT four-box model (thoughts / feelings /
-- behaviours / bodily sensations) around a trigger. A light "observe" practice.

create table if not exists public.noticing_logs (
  ...
  bodily_sensations text not null default '',
  logged_at timestamptz not null default timezone('utc', now()),
  created_at timestamptz not null default timezone('utc', now())
);

create index if not exists noticing_logs_user_logged_idx
  on public.noticing_logs (user_id, logged_at desc);

alter table public.noticing_logs enable row level security;

drop policy if exists "noticing_logs_select_own" on public.noticing_logs;
create policy "noticing_logs_select_own" on public.noticing_logs
  for select to authenticated using (auth.uid() = user_id);

drop policy if exists "noticing_logs_insert_own" on public.noticing_logs;
create policy "noticing_logs_insert_own" on public.noticing_logs
  for insert to authenticated with check (auth.uid() = user_id);

drop policy if exists "noticing_logs_update_own" on public.noticing_logs;
create policy "noticing_logs_update_own" on public.noticing_logs
  for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "noticing_logs_delete_own" on public.noticing_logs;
create policy "noticing_logs_delete_own" on public.noticing_logs
  for delete to authenticated using (auth.uid() = user_id);

-- Extend account data export to include noticing logs.
...
begin
  ...
  select json_build_object(
    ...
    'noticingLogs', (
      select coalesce(json_agg(row_to_json(nl)), '[]'::json)
      from (
        select
          id,
          situation,

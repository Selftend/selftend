...
--
-- The whole flat function is redeclared here, per supabase/README.md
-- "Modifying export_user_data": it enumerates meditation columns explicitly and
-- stopped at `technique_used`, so without this the export would hand back
-- `completed_at` with no way to reconstruct the civil day the sit belonged to -
-- which is the one fact this migration exists to record. An export that cannot
-- distinguish a captured offset from a legacy null is not a complete export.
--
-- Copied verbatim from 20260717_routine_scheduling.sql with a single line added
-- to the meditationSessions projection.

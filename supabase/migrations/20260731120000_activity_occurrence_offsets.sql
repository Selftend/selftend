-- Capture the civil day a behavioural-activation activity was completed on, and the
-- civil day it was planned for (#330).
--
-- TWO columns, for two different reasons. This is the one table in #330 that needs
-- both, and conflating them would get one of them wrong:
--
--   completed_at -> completed_offset_minutes
--     The standard occurrence case, identical to meditation (20260729) and
--     breathing/grounding (20260730). "I did it on Tuesday" is a fact about Tuesday
--     in the place it happened, and travel must never move it.
--
--   scheduled_at -> scheduled_offset_minutes
--     A user-picked FUTURE local time, kept for a different reason (owner decision,
--     2026-07-28). Behavioural activation asks "did I do the thing I planned for that
--     day": the day is the unit of the intervention, and "Tuesday 7pm" means your
--     Tuesday evening, not an instant shared with anyone else. So the intended civil
--     day is frozen at the moment of planning too.
--
--     This does NOT walk back src/features/activities/schedule-format.ts:12-15, which
--     defends storing an unambiguous ISO instant rather than tz-naive text. That stays
--     true and unchanged: the instant still does the ordering and the time-of-day
--     display. The offset is ADDED alongside it to record which civil day was meant.
--     Two facts about one plan, both stored, neither derivable from the other.
--
-- Nullable, no default, no backfill - both columns. This is the shape
-- 20260726_occurrence_offset_nullable had to retrofit onto the first four tables after
-- `not null default 0` made every pre-existing row claim UTC. A stored value is only
-- ever one a client explicitly sent; rows written before this migration record
-- "unknown" and keep falling back to the viewer's local day, which is exactly where
-- they render today, so nothing moves on deploy.
alter table public.activity_logs_data
  add column if not exists completed_offset_minutes smallint
    check (completed_offset_minutes between -840 and 840);

alter table public.activity_logs_data
  add column if not exists scheduled_offset_minutes smallint
    check (scheduled_offset_minutes between -840 and 840);

-- The view is the client's only handle on this table - PostgREST cannot see a column
-- that is missing here. Re-list every column the view already exposes (20260621, less
-- the plaintext pair dropped in 20260622) and append the two new ones.
...
-- Both offsets are written by the encrypted-view writers themselves rather than by a
-- supplemental UPDATE from the occurrence trigger (the 20260708:98-115 shape). That is
-- the same choice 20260730 made, for a different reason: mindfulness_sessions_data has
-- no UPDATE policy at all, whereas activity_logs_data does (activity_logs_update_own,
-- 20260667:33) and a supplemental UPDATE would in fact land here. Folding into the
-- writers is still the better shape - one write instead of two, and completed_at moves
-- on UPDATE (completeActivity) as well as INSERT, so the offset has to travel with the
-- writer that carries the timestamp rather than chase it.
...
begin
  ...
  insert into public.activity_logs_data (
    ...
    completed_offset_minutes, scheduled_offset_minutes)
  values (
    ...
    -- No `coalesce(..., 0)` (20260726:81-84): a client that omits either column
    -- records "unknown", not an assertion that it was standing at UTC.
    new.completed_offset_minutes, new.scheduled_offset_minutes)
...
-- Rebased on the 20260662 definition (which added the RETURNING that refreshes
-- updated_at/created_at from the BEFORE-UPDATE trigger), not on 20260621's - taking the
-- older copy would silently undo that fix.
...
begin
  ...
  update public.activity_logs_data set
    ...
    scheduled_offset_minutes = new.scheduled_offset_minutes
...
-- triggers in name order, so this sorts after `activity_logs_ins` / `_upd` and runs
-- once they have resolved NEW.
--
-- Only completed_at goes through validate_occurrence_time, and only when there is one:
--
--   * The column is nullable by design - a scheduled-but-not-yet-done activity has no
--     completion - and validate_occurrence_time raises 'Occurrence time is required'
--     on a null. Calling it unconditionally would reject every activity at creation.
--   * scheduled_at CANNOT go through it at all: it rejects anything more than five
--     minutes in the future (20260726:71-74), which is precisely what a schedule is.
--     Its offset is range-checked directly instead - the same rule, minus the
--     not-in-the-future clause that does not apply to an intention.
--
-- validate_occurrence_time has tolerated a null offset since 20260726:59-79, so an
-- uncaptured completion offset passes while an out-of-range one still raises.
create or replace function public.zz_activity_logs_occurrence() returns trigger
language plpgsql security invoker set search_path = pg_catalog, public as $$
begin
  if new.completed_at is not null then
    perform public.validate_occurrence_time(new.completed_at, new.completed_offset_minutes);
  end if;
  if new.scheduled_offset_minutes is not null
     and (new.scheduled_offset_minutes < -840 or new.scheduled_offset_minutes > 840) then
    raise exception 'Invalid occurrence offset' using errcode = 'check_violation';
  end if;
  return new;
end;
$$;
drop trigger if exists zz_activity_logs_occurrence_ins on public.activity_logs;
create trigger zz_activity_logs_occurrence_ins instead of insert on public.activity_logs
  for each row execute function public.zz_activity_logs_occurrence();
drop trigger if exists zz_activity_logs_occurrence_upd on public.activity_logs;
create trigger zz_activity_logs_occurrence_upd instead of update on public.activity_logs
  for each row execute function public.zz_activity_logs_occurrence();

-- ── export_user_data: carry both captured offsets into the GDPR export ───────
--
-- The whole flat function is redeclared here, per supabase/README.md "Modifying
-- export_user_data". The activityLogs projection enumerates columns explicitly, so
-- without this the export would hand back scheduled_at and completed_at with no way to
-- reconstruct either civil day - the two facts this migration exists to record.
--
-- Copied from 20260730_mindfulness_occurrence_offset.sql, NOT from the older definition
-- in 20260717. Every migration that adds a column redeclares this whole function and
-- the last one to run wins, so starting from an older copy would silently drop
-- meditation's and mindfulness's completed_offset_minutes back out of the export.
-- Nothing asserts export completeness column-by-column, so that would not be caught.
-- The only difference from 20260730 is the two lines added to activityLogs below.

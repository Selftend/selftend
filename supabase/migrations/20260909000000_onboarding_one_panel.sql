-- Onboarding becomes one panel; `selected_concerns` and `widgets_seeded` are
-- dropped (#1958, spec #1885 §5-5.2, decision #1891).
--
-- WHAT CHANGED IN THE APP. The first-run wizard is the welcome panel alone. The
-- concerns, modules and guidance panels are gone, and with them the only product
-- consumer of `selected_concerns` (the wizard pre-ticking its own boxes) and the
-- only reason `widgets_seeded` existed (onboarding no longer seeds Home - a new
-- user lands on empty Favourites over the full catalogue). Completion is written
-- by the plain preference update the protected layout already made for the
-- introduction replay, now carrying `app_onboarding_completed_via` and `_at` as
-- well as the flag. The app calls `apply_widget_recommendations` from nowhere.
--
-- WHY THE FUNCTION IS REDECLARED RATHER THAN DROPPED. Native builds have no OTA
-- channel, so every shipped build still calls `apply_widget_recommendations`
-- with all three arguments when its wizard finishes. Its live declaration wrote
-- `widgets_seeded = true` and `selected_concerns`; dropping the columns under it
-- would turn onboarding-finish on every shipped build into a raw SQL error. So
-- the SAME SIGNATURE is kept - `p_selected_concerns` is still accepted, and it
-- still feeds `initial_concerns` under the write-once guard exactly as before -
-- and the two dropped columns are simply no longer touched. Old builds keep
-- writing `widget_preferences`, which the current app never reads; the
-- retirement condition for the table and its four functions is recorded in
-- supabase/README.md and is not scheduled.
--
-- THE ATOMICITY ARGUMENT IN 20260707's HEADER IS DEAD. That header justified
-- the function as "one transaction: a client disconnect cannot leave Home
-- changed while onboarding remains incomplete". With the seeding half gone from
-- the app there is no Home change left to be atomic with; the function survives
-- for old callers only, and the transaction now guards a list nothing current
-- renders.
--
-- `initial_concerns` KEEPS EVERY ROW AND IS NEVER WRITTEN AGAIN BY THE APP. Not
-- the same object as `selected_concerns`: it is the immutable intake record
-- (#1612), read by scripts/analytics-segment.sql, and its concern arm now covers
-- the pre-redesign cohort only. Asking every new user to declare mental-health
-- concerns purely to feed a retention report was refused (#1891).
--
-- EXISTING USERS LOSE THEIR STORED `selected_concerns`. Accepted (#1891): its
-- only reader was the wizard's pre-tick, and the wizard no longer asks.
...
begin
  ...
  -- `p_selected_concerns` is accepted for the old caller's sake and feeds ONLY
  -- the intake record below; `selected_concerns` and `widgets_seeded` no longer
  -- exist (#1958).
...
-- The columns go AFTER the function stops naming them. plpgsql bodies are not
-- dependency-tracked, so the order is for the reader, not for Postgres.
alter table public.user_preferences
  drop column if exists selected_concerns,
  drop column if exists widgets_seeded;

comment on column public.user_preferences.initial_concerns is
  'Concerns declared at first onboarding completion. Immutable: written once by apply_widget_recommendations and never updated, including for accounts grandfathered in by 20260705 whose app_onboarding_completed_at is NULL (#1648). NULL means never recorded, which is an explicit "unknown" arm rather than "no concerns". Since #1958 the app no longer asks: only native builds that predate the one-panel onboarding still call the function, so the column covers the pre-redesign cohort only.';

-- `export_user_data`, redeclared from the live declaration
-- (20260908000000_favorites.sql) minus the two dropped columns and nothing else.
-- Both are named in test/export-user-data-monotonic.test.ts's INTENTIONALLY_DROPPED
-- list, the way `plan_items` was retired.

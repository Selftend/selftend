-- Capture the civil day a thought record was written on (#330).
--
-- A thought record is a diary entry about the moment you caught a thought, so
-- the day it belongs to is the day it was at the place you wrote it - it should
-- never move because you travelled afterwards. Today `created_at` is bucketed
-- through the viewer's current timezone, so a record written at 06:00 Monday in
-- Tokyo (21:00 Sunday UTC) reads as Sunday's once you land in London.
--
-- `created_offset_minutes`, NOT `occurred_at` (owner decision, 2026-07-28). The
-- editor has no date field and none is wanted: journal carries a separate
-- `occurred_at` precisely because journal entries are back-datable, and a thought
-- record is not. This is the offset OF THE CREATION INSTANT, read as
-- `entryDayKey(created_at, created_offset_minutes)`. If back-dating is ever
-- asked for it upgrades cleanly to `occurred_at` + `occurred_offset_minutes`.
--
...
-- already falls back to the viewer's local day for it - which is exactly where
-- those rows render today, so nothing moves on deploy. From here on a stored 0
-- is only ever a value a client explicitly sent, so it means what it says.
--
...
alter table public.thought_records_data
  add column if not exists created_offset_minutes smallint
...
-- column that is missing here. Re-list every column the view already exposes
-- (20260591, whose plaintext originals 20260592 then dropped from the base
-- table) and append the new one.
...
-- The offset is written by the encrypted-view writers themselves rather than by
-- a supplemental UPDATE from the occurrence trigger (the 20260708:98-115 shape),
-- following 20260730_mindfulness_occurrence_offset. Unlike mindfulness_sessions
-- the supplemental UPDATE would in fact work here - thought_records_data has a
-- `thought_records_update_own` policy (20260415:124, re-pointed at the _data
-- table by 20260667:123) - but it would cost a second write per save and, on the
-- INSERT path, fire the `set_thought_records_updated_at` BEFORE-UPDATE trigger,
-- stamping a brand-new record as already edited. The history list sorts and
-- labels by `updated_at`, so that is user-visible. Folding the column into the
-- writers avoids both.
--
-- Copied from 20260591 with the single column added; the 4000-character caps
-- live here (20260592 dropped the CHECK constraints along with the plaintext).
...
-- Kept in step with the INSERT writer. An UPDATE that does not mention the
-- column - editing a record's text, or archiving it - carries the stored value
-- through NEW unchanged, so a record's captured day survives every later edit.
...
-- The `zz_` prefix is load-bearing, not style. PostgreSQL fires row-level
-- INSTEAD OF triggers in name order, so this sorts after `thought_records_ins` /
-- `_upd` and runs once they have resolved NEW. That matters because `created_at`
-- is server-defaulted and a client may omit it: only after the INSERT writer has
-- returned the stored value into NEW is there an occurrence time to validate at
-- all. Renaming this to sort earlier would reject every such insert as
...
create or replace function public.zz_thought_records_occurrence() returns trigger
language plpgsql security invoker set search_path = pg_catalog, public as $$
begin
  perform public.validate_occurrence_time(new.created_at, new.created_offset_minutes);
  return new;
end;
$$;
drop trigger if exists zz_thought_records_occurrence_ins on public.thought_records;
create trigger zz_thought_records_occurrence_ins instead of insert on public.thought_records
  for each row execute function public.zz_thought_records_occurrence();
drop trigger if exists zz_thought_records_occurrence_upd on public.thought_records;
create trigger zz_thought_records_occurrence_upd instead of update on public.thought_records
  for each row execute function public.zz_thought_records_occurrence();

-- ── export_user_data: carry the captured offset into the GDPR export ─────────
--
-- The whole flat function is redeclared here, per supabase/README.md "Modifying
-- export_user_data". The thoughtRecords projection enumerates columns explicitly
-- and stopped at `updated_at`, so without this the export would hand back
-- `created_at` with no way to reconstruct the civil day - the one fact this
-- migration exists to record, and the one an export cannot be complete without.
--
-- Copied from 20260730_mindfulness_occurrence_offset.sql, which is the newest
-- declaration, NOT from any older one. Several migrations now redeclare this
-- whole function and each runs in filename order, so starting from an older copy
-- would silently drop meditation's and mindfulness's `completed_offset_minutes`
-- back out of the export. Nothing asserts export completeness column-by-column,
-- so that would not be caught. The only difference from 20260730 is the one line
-- added to the thoughtRecords projection.

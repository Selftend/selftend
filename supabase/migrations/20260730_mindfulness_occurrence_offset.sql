...
-- export_user_data". The mindfulnessSessions projection enumerates columns
-- explicitly and stopped at `created_at`, so without this the export would hand
-- back `completed_at` with no way to reconstruct the civil day - the one fact
-- this migration exists to record, and the one an export cannot be complete
-- without.
--
-- Copied from 20260729_meditation_occurrence_offset.sql, NOT from the older
-- definition in 20260717. These two migrations both redeclare the whole
-- function and this one runs second, so starting from the older copy would
-- silently drop meditation's `completed_offset_minutes` back out of the export.
-- The only difference from 20260729 is the one line added below.

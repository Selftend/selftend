-- Age attestation: the verdict of the age gate, and nothing more (#1762, spec
-- #227 §3).
--
-- The gate asks a date of birth and a country, compares them against the
-- per-country floor (src/features/auth/age-floor.ts, #1761) and keeps the
-- ANSWER. The date of birth is discarded and has no column here - that is the
-- whole of §3's "DOB is discarded", and a column would quietly undo it. What
-- survives is: did they clear their country's floor, which country they said
-- they were in, and when they said it.
--
-- All three are NULLABLE with no default, and that is load-bearing. NULL means
-- "never asked", which is the true state of every account that exists today and
-- will REMAIN the true state of every one of them: §7 gives existing users the
-- one-time consent prompt without re-asking age or country. A `not null default
-- false` on age_floor_met would instead assert that every current user FAILED
-- their floor, and a later gate reading `= true` would lock out the whole
-- install base. Three states is the honest shape - unknown, met, not met - and
-- only the middle one is a pass.
--
-- age_attested_country is checked against ^[A-Z]{2}$ rather than left free, and
-- the check leans the safe way on purpose: an unrecognised code falls through to
-- the floor table's catch-all of 13, the LOWEST floor, so junk in this column
-- silently UNDER-protects. Failing the write is the better error. The gate
-- normalises to upper-case ISO 3166-1 alpha-2 before storing.
--
-- No RLS changes. user_preferences' policies are row-level and scope by
-- user_id, so they already cover every column the row gains; a per-column
-- policy would be new machinery for nothing. Stated explicitly because "adds no
-- policy" should read as a decision rather than an omission.
--
-- export_user_data is redeclared with the three new columns added to the main
-- preferences projection, beside the other consent fields. Rebuilt from
-- 20260904000000, the newest declaration on dev, with ONLY those lines added -
-- verified by diff. They are exported rather than withheld because
-- supabase/README.md's withholding rule earns an exemption only for caller
-- scoping, credentials, server bookkeeping or plumbing, and user_preferences
-- withholds nothing but user_id: a country the person typed and the timestamp of
-- their own act are plainly their data. Versioned 20260905000000: full 14
-- digits, later than every existing version, sharing a prefix with none
-- (supabase/README.md "Migration versions").

alter table public.user_preferences
  add column if not exists age_floor_met boolean,
  add column if not exists age_attested_country text,
  add column if not exists age_attested_at timestamptz;

-- Dropped first so the migration is re-runnable against a database that already
-- has the constraint; `add constraint` has no `if not exists`.
alter table public.user_preferences
  drop constraint if exists user_preferences_age_attested_country_check;

alter table public.user_preferences
  add constraint user_preferences_age_attested_country_check
  check (age_attested_country is null or age_attested_country ~ '^[A-Z]{2}$');

comment on column public.user_preferences.age_floor_met is
  'Whether the person cleared their country''s minimum-age floor at the age gate (#1762, spec #227 §3). NULL means never asked - the state of every account predating the gate, and the state they keep. The date of birth that produced this verdict is never stored.';

comment on column public.user_preferences.age_attested_country is
  'Self-declared country of residence given at the age gate, upper-case ISO 3166-1 alpha-2 (#1762). Selects the floor in src/features/auth/age-floor.ts. NULL means never asked.';

comment on column public.user_preferences.age_attested_at is
  'When the age attestation was made (#1762). NULL means never asked.';

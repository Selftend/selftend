-- Squash the export_user_data wrapper chain (10 shadows) into one flat
-- definition. Output is byte-identical to the previous chain (proven by
-- jsonb-equality capture on seeded users during development).
--
-- CONVENTION FROM NOW ON: extend this function by editing THIS flat definition
-- with `create or replace` in a new migration. Do NOT wrap it in another
-- export_user_data_before_* shadow. Whenever a user-data column or table is
-- added, add it here (GDPR export completeness).
--
-- Reconstructed top-down from the live call chain:
--   export_user_data
--    -> _before_funnel_columns   (devicePushTokens; funnel prefs merge lives in head)
--     -> _before_push_tokens     (reminder prefs merge into preferences)
--      -> _before_tool_reminders (act* program state/logs, planItems, widgetPreferences)
--       -> _before_act_plan_widget (emotionPreferences)
--        -> _before_emotion_prefs  (meditation*, gratitudeEntries, habits, habitLogs)
--         -> _before_module_restore (LEAF: the base json_build_object)
-- The other four *_before_* functions (gratitude_entries, gratitude_phase4_5,
-- habits, meditation_tmi) were orphaned when _before_module_restore re-inlined
-- their tables; they are dropped below too.

-- The previous head returned `json`; `create or replace` cannot change a
-- function's return type, so drop it first before recreating as `jsonb`.
...
begin
  ...
  -- === base (was export_user_data_before_module_restore) ===
  ...
  -- === meditation / gratitude / habits (was export_user_data_before_emotion_prefs) ===
  ...
  -- === emotion preferences (was export_user_data_before_act_plan_widget) ===
  result := result || jsonb_build_object(
    'emotionPreferences', (
  ...
  -- === ACT program state / logs (was export_user_data_before_tool_reminders) ===
  result := result || jsonb_build_object(
    'actProgramState', (
      select to_jsonb(s)
      from (
  ...
  -- === reminder prefs merged into preferences (was export_user_data_before_push_tokens) ===
  ...
  -- === device push tokens (was export_user_data_before_funnel_columns) ===
  result := result || jsonb_build_object(
    'devicePushTokens', (
...
-- Drop every shadow (10 total, enumerated from pg_proc where proname like
-- 'export_user_data_before%'). Six were live chain links; four were orphaned
-- when _before_module_restore re-inlined their tables. One drop per shadow:
drop function if exists public.export_user_data_before_funnel_columns();
drop function if exists public.export_user_data_before_push_tokens();
drop function if exists public.export_user_data_before_tool_reminders();
drop function if exists public.export_user_data_before_act_plan_widget();
drop function if exists public.export_user_data_before_emotion_prefs();
drop function if exists public.export_user_data_before_module_restore();
drop function if exists public.export_user_data_before_gratitude_entries();
drop function if exists public.export_user_data_before_gratitude_phase4_5();
drop function if exists public.export_user_data_before_habits();
drop function if exists public.export_user_data_before_meditation_tmi();

-- Guard: fail loudly if any export_user_data_before_* shadow function still
-- exists. Runs at every db:reset / db push so the squash cannot be silently
-- re-wrapped by a later migration.
do $$
begin
  if exists (select 1 from pg_proc where proname like 'export_user_data_before%') then
    raise exception 'wrapper shadows remain: export_user_data_before_* functions must not exist after this migration';
  end if;
end $$;

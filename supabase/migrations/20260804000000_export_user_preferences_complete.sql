-- Complete the `preferences` export: every user_preferences column the user
-- set now goes out with their data (#429 bucket 3, decided 2026-07-28).
--
-- The rule this lands, recorded in supabase/README.md "What the export
-- deliberately withholds": a column is exported unless it is caller scoping
-- (user_id), a credential/secret, server bookkeeping, or internal plumbing -
-- and every withheld column is named there with its reason. "It is app state"
-- is not on that list: a theme, a chosen ambient sound, an onboarding step or
-- a program phase is data the user provided or generated, and the export is
-- the user's own authenticated view of their account, which already carries
-- far more sensitive content than any of these.
--
-- Thirty-four columns join the main preferences projection: theme, sound and
-- volume choices (ambient/breath sound + volume, breathing cycles, last
-- breathing pattern), notifications_enabled_global, the per-tool
-- onboarding/first-run flags (mood, gratitude, journal, sleep, habits,
-- meditation + info, mindfulness, grounding, act, cbt wizard), seeded/tour
-- state (emotions_seeded, widgets_seeded, shown_button_tours), and the CBT and
-- ACT programme state (started/phase/completed/prompt-dismissed/graduation).
--
-- Still withheld from user_preferences after this change: user_id only.
--
-- Two whole tables also join the export, both caught by the table-level half
-- of the new completeness check (every live table with a user_id column must
-- be read by this function or named in the README's withheld list):
--
--   breathing_exercises      user-created custom exercises - name, four phase
--                            timings, cycles, colour. Content, plainly.
--   feedback_submissions     (id, created_at) rate-limit rows. Bookkeeping in
--                            purpose, but each row records that THIS user sent
--                            feedback at THAT moment, which is their activity.
--
-- Rebuilt from 20260802000000, the newest declaration on dev, with ONLY the
-- lines above added - verified by diff against that file. Versioned
-- 20260804000000: a full 14-digit version, later day than every existing
-- version, sharing a prefix with none (supabase/README.md "Migration
-- versions").

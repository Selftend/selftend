-- Add optional thought record fields for evidence and emotion shift tracking.

alter table public.thought_records
  add column if not exists emotion_intensity_before integer check (emotion_intensity_before between 0 and 100),
  add column if not exists evidence_for text[] not null default array[]::text[],
  add column if not exists evidence_against text[] not null default array[]::text[],
  add column if not exists emotion_intensity_after integer check (emotion_intensity_after between 0 and 100),

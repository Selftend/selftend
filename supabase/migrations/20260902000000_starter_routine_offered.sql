-- The starter-routine offer's once-ever shown-flag (#1677, decided in #1663).
--
-- The offer itself is derived from the record at save time (#952) and stores
-- nothing: no routine + records in a second distinct tool = eligible. The one
-- fact that cannot be derived is "this person has already been asked", exactly
-- as with the reminder prompt's `reminder_prompted_tools` - this flag is that
-- list's once-ever boolean twin, written the moment the card is shown, so
-- navigating away counts as asked and declining writes nothing further.

alter table public.user_preferences
  add column if not exists starter_routine_offered boolean not null default false;

comment on column public.user_preferences.starter_routine_offered is
  'The once-ever starter-routine offer at the second action (#1677) has been shown. Marked on show: navigating away counts as asked, and declining writes nothing further.';

-- Every migration adding an exportable user_preferences column redeclares the
-- whole flat export_user_data (supabase/README.md); this declaration differs
-- from 20260901000000_initial_concerns.sql by exactly the
-- starter_routine_offered line (and the trailing comment naming this column).
...
-- reload `starter_routine_offered` stays invisible to the client until the cache

-- Persist the meditation Today card's interval-bell choice (#1190).
--
-- The card's two controls both forgot. Sit length already had a home -
-- meditation_program_state.preferred_duration_minutes, written until now only
-- by the onboarding wizard - so it needed no schema at all. The interval bell
-- had none: bellMinutes was useState(0), handed to the sit screen as a route
-- param and stored nowhere, so it reset to "off" on every visit.
--
-- It lands on user_preferences rather than meditation_program_state because it
-- is a playback preference, not programme progress: the four breathing audio
-- columns are its neighbours and its exact template, and #1188's bell volume
-- will want to sit beside it rather than be split across two tables.
--
-- Nullable with no default, mirroring breathing_cycles: null means "never
-- chosen", which the app reads as 0 - the bell stays off until asked for.
-- No CHECK, deliberately - #1189 is still deciding whether a half-time bell is
-- a minute value or a mode, and a constraint written now would prejudge it.
--
-- export_user_data is redeclared with the one new column added to the main
-- preferences projection. Rebuilt from 20260811000000, the newest declaration
-- on dev, with ONLY that line added - verified by diff. Versioned
-- 20260820000000: full 14 digits, later than every existing version, sharing a
-- prefix with none (supabase/README.md "Migration versions").

alter table public.user_preferences
  add column meditation_interval_bell_minutes integer;

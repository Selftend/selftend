-- The general reminder's columns (#2489; spec `docs/reminders.md` § 3.1 and § 7.1;
-- rule in ADR-0010).
--
-- A twelfth reminder target, `general`: the one reminder that leads to the app
-- rather than to a tool or a routine. Nothing here arms it - every column
-- defaults to the off state, existing rows take the defaults, and nothing is
-- backfilled. The client and the cron learn the target in a later change; until
-- then these columns are read by nothing, which is why the migration can land
-- first: a shipped client that does not know a column never sends it.
--
-- Default hour 18 - a free slot before the 19:00 module cluster, so a person
-- with a module reminder on is not nudged twice within a minute by default
-- (#2413). The four-column shape and the two dedup keys follow the DBT
-- template (20260910000000) exactly, so every hand-listed reader that names
-- `<key>_reminders_enabled` / `_hour` / `_minute` / `_timezone` and
-- `last_<key>_reminder_key` takes the new prefix with no other change.

alter table public.user_preferences
  add column if not exists general_reminders_enabled boolean not null default false,
  add column if not exists general_reminder_hour int not null default 18
    check (general_reminder_hour between 0 and 23),
  add column if not exists general_reminder_minute int not null default 0
    check (general_reminder_minute between 0 and 59),
  add column if not exists general_reminder_timezone varchar;

-- Per-target dedupe keys, on both push channels (20260542, 20260583): one
-- notification a day per channel, the only suppression the general target has.
-- Withheld from the export by the existing `*.last_*_reminder_key*` rule in
-- `supabase/README.md` - server bookkeeping, not user data.
alter table public.web_push_subscriptions
  add column if not exists last_general_reminder_key text;
alter table public.device_push_tokens
  add column if not exists last_general_reminder_key text;

-- ═══════════════════════════════════════════════════════════════════════════════
-- export_user_data - redeclared from 20260915000000 plus the four general columns
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- fails on any live column that is neither exported nor README-withheld, and
-- `test/export-user-data-monotonic.test.ts` fails if a later declaration loses
-- any of them.
...
-- PostgREST caches the schema, and this migration ADDS columns a client shipped
-- with it will read and write. Without the reload a fresh client's first upsert
-- of the new columns meets a stale cache that does not list them and fails at
-- the database instead; the reload is what lets the columns be seen.

-- Store an encrypted value key on a goal (#1287).
--
-- A goal may be anchored to ONE of the user's own ranked priority values. The key
-- answers "what am I being"; life_domain answers "where in my life". Both are kept -
-- life_domain is non-null on a table with existing rows, so removing it would be a real
-- migration for a redundancy that is not there.
--
-- NULLABLE AT EVERY LAYER, ON PURPOSE. The shipped programme's first week lists
-- SET_GOALS before CLARIFY_VALUES, so on the intended path a user writes their first
-- goal with no priority values in existence. A required column would be unfillable at
-- exactly the moment the programme asks for a goal. Nothing here may gate goal creation.
--
-- ENCRYPTED, DELIBERATELY OVERRIDING THIS TABLE'S OWN CONVENTION. goals has been a
-- transparent encrypted view over goals_data since 20260625, but only title and
-- description are encrypted; life_domain, goal_type, status and target_date sit in
-- plaintext. By that convention the value key would be plaintext too. It is encrypted
-- anyway, because values_profile encrypts its own value list (20260629) even though that
-- is likewise fixed-key enum data - which values someone holds was judged sensitive. A
-- plaintext pointer would put "anchored to honesty" in the clear and partly defeat that
-- decision. The pointer is protected at least as well as the thing it points at. The
-- table's own convention was derived from columns like status and target date, which
-- carry nothing personal.
--
-- NO BACKFILL, unlike 20260821010000's note field. That column seeded existing rows with
-- an encrypted empty string because every encrypted text column on that view reads back
-- as a non-null string. Here NULL is meaningful and must survive: it is "anchored to
-- nothing", which is distinct from "anchored to the empty string". app.encrypt_text and
-- app.decrypt_text both map NULL to NULL (20260586), so the round trip is faithful as
-- long as no coalesce is introduced - exactly how gratitude_entries_upd handles its
-- optional slots in 20260662.
--
-- Both INSTEAD OF trigger functions are re-declared in full from their winning
-- declarations, not patched: goals_ins from 20260625, and goals_upd from 20260662 (which
-- added the RETURNING of fresh timestamps - carried over here verbatim).
--
-- export_user_data() is re-declared in full, rebuilt from 20260821010000, the newest
-- declaration on dev, with ONLY value_key added to the goals projection - verified by
-- diff. Versioned 20260821020000: full 14 digits, later than every existing version,
-- sharing a prefix with none (supabase/README.md "Migration versions").
--
-- One-directional by design: no reverse view of anchored goals on the values screen, and
-- nothing consumes the key yet - not the weekly review, not the recovery plan, not the
-- programme signal. The two surfaces that read it follow in #1289 and #1291.

-- === Step A: ciphertext column on the base table ===
-- No backfill: existing goals are anchored to nothing, and NULL says so.
alter table public.goals_data add column if not exists value_key_enc bytea;

-- === Step B: the decrypting view gains the column ===
-- APPENDED after updated_at, not slotted in beside life_domain: `create or replace view`
-- may only ADD columns at the end - reordering raises "cannot change name of view
-- column". The client reads the view by name (select *), so position carries no meaning.
-- Dropping and recreating the view instead would take the INSTEAD OF triggers and the
-- grants with it for no gain.
create or replace view public.goals with (security_invoker = true) as
  select id,
         ...
         created_at,
         updated_at,
         app.decrypt_text(value_key_enc)   as value_key
  from public.goals_data;

-- === Step C: both INSTEAD OF trigger functions encrypt the key on write ===
-- NOTE the absence of coalesce on value_key in both: a coalesce to '' would turn "not
-- anchored" into "anchored to the empty string" and the null case would stop round-
-- tripping. title and description keep theirs - they are NOT NULL on the view's contract.
...
begin
  ...
  insert into public.goals_data (
    ...
    target_date, status, value_key_enc, created_at, updated_at)
  values (
    ...
    app.encrypt_text(new.value_key),   -- NO coalesce: an unanchored goal stays NULL
...
begin
  ...
  update public.goals_data set
    ...
    status          = new.status,
    value_key_enc   = app.encrypt_text(new.value_key),  -- NO coalesce, as above
...
-- Rebuilt from 20260821010000 with ONLY value_key added to the goals projection, so
-- every column that migration added to selfCareLogs travels forward untouched. Every
-- migration adding an exportable column re-declares the WHOLE body; this is never a patch.

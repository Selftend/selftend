-- Favourites (#1953, spec #1885 §4, decision #1889): a tool or a module the person
-- starred to keep on Home. A NEW table, and every existing dashboard row becomes
-- one of these - once.
--
-- WHY A SECOND TABLE. `widget_preferences` is not touched, and neither are the
-- four server functions that write its `position` (`add_widget_preference`,
-- `set_widget_order`, `normalize_widget_positions`, `apply_widget_recommendations`).
-- Native builds have no OTA channel, so every shipped build keeps calling those
-- four against the old shape indefinitely; reshaping the table would turn a
-- shipped app's add button into an error. A second table costs this copy plus
-- three doc entries, and an old phone keeps a WORKING dashboard. The old table
-- and its functions now serve only pre-Favourites native builds; their
-- retirement condition is recorded in supabase/README.md and not scheduled.
--
-- THE SHAPE. `(user_id, kind, key)` with `kind in ('tool', 'module')` - one of the
-- 8 tool hubs on /tools or one of the 3 modules on /modules. No `position`: Home
-- renders a fixed array of eleven ids and Favourites is that array FILTERED, so
-- there is no order to store. No extra index: the unique constraint's btree
-- serves the only query (`where user_id = ?`). No RPC and no security definer:
-- add is `insert … on conflict do nothing`, remove is a `delete`, both under the
-- caller's own RLS. `key` is deliberately unconstrained - an unrecognised key is
-- ignored on read, which is what lets a future tool or module land without a
-- migration and what keeps a downgraded build from erroring on a row it does not
-- know.
--
-- RLS carries BOTH `using` and `with check`. The old table's policy has only
-- `using` (Postgres reuses it for `for all`, so it is correct but asymmetric);
-- that asymmetry is not inherited.

create table public.favorites (
  id         uuid        primary key default gen_random_uuid(),
  user_id    uuid        not null references auth.users(id) on delete cascade,
  kind       text        not null check (kind in ('tool', 'module')),
  key        text        not null,
...
comment on table public.favorites is
  'A tool (/tools hub) or module (/modules) the person starred to keep on Home (#1953). One row per (kind, key); no ordering - Home renders its fixed catalogue order filtered to these. Replaces widget_preferences for every build that ships Favourites; the old table stays for older native builds.';
...
-- THE ONE-SHOT COPY. Every `widget_preferences` row becomes the favourite its
-- widget's `toolKey` names - the mapping WIDGET_META carries in TypeScript,
-- restated here in SQL and then SPENT: no CHECK rejects the old ids, nothing
-- remaps at read time, and old builds keep writing the old table, which the new
-- app never reads. So nothing has to stop the two vocabularies drifting; after
-- this statement runs they are simply unrelated.
--
-- 25 ids → 11 possible favourites, unevenly (#1889):
--   8 shared tools map 1:1 onto `tool:<key>`;
--   the 9 CBT ids (2 module homes + 7 exercise doors) collapse to `module:cbt`;
--   the 7 ACT ids collapse to `module:act`;
--   `routines-today` is DROPPED - /routines is on neither list;
--   nothing reaches `module:dbt`, because no widget id ever carried it.
-- A fully loaded dashboard lands at 10 of 11. Deliberately no cap and no
-- threshold behaviour (#1910).
--
...
-- because migrations have already applied by the time any test runs. Keep the
-- statement, the whole statement and nothing else between them.
...
select distinct wp.user_id, m.kind, m.key
  from public.widget_preferences as wp
  join (values
    -- shared tools: 8, one each
    ('mood-checkin',         'tool',   'mood'),
    ('breathing-suggested',  'tool',   'breathing'),
    ('gratitude-latest',     'tool',   'gratitude'),
    ('meditation-pick',      'tool',   'meditation'),
    ('habits-today',         'tool',   'habits'),
    ('sleep-latest',         'tool',   'sleep'),
    ('journal-week',         'tool',   'journal'),
    ('grounding-log',        'tool',   'grounding'),
    -- cbt: 9 → 1
    ('self-care',            'module', 'cbt'),
    ('cbt-open-record',      'module', 'cbt'),
    ('cbt-distortion-guide', 'module', 'cbt'),
    ('cbt-programme',        'module', 'cbt'),
    ('cbt-worry',            'module', 'cbt'),
    ('cbt-beliefs',          'module', 'cbt'),
    ('cbt-activities',       'module', 'cbt'),
    ('cbt-exposure',         'module', 'cbt'),
    ('cbt-goals',            'module', 'cbt'),
    -- act: 7 → 1
    ('act-drop-anchor',      'module', 'act'),
    ('act-observing-self',   'module', 'act'),
    ('act-choice-point',     'module', 'act'),
    ('act-programme',        'module', 'act'),
    ('act-committed-actions','module', 'act'),
    ('act-defusion',         'module', 'act'),
    ('act-acceptance-prompt','module', 'act')
    -- 'routines-today' is absent on purpose: dropped, see above.
  ) as m(widget_id, kind, key)
    on m.widget_id = wp.widget_id
on conflict (user_id, kind, key) do nothing;
-- <<< one-shot copy

-- Redeclared IN FULL with a `favorites` projection added beside
-- `widgetPreferences`, per supabase/README.md "Modifying export_user_data",
-- copied from 20260906000000_health_data_consent.sql - the newest declaration
-- on dev, NOT the age-attestation one the decision tickets cite. This only adds,
-- so test/export-user-data-monotonic.test.ts is satisfied by construction; the
-- completeness integration test is what requires the new table to be here at
-- all.
--
-- Version 20260908000000: strictly after 20260907000000_record_days.sql, the
-- newest migration on dev when this was written (supabase/README.md "Migration
-- versions").
...
-- PostgREST caches the schema, and this migration adds a table. Without the

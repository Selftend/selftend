-- Module introductions are opened explicitly from each module's info action.
-- These eleven columns had no read-side consumer, so retaining them collected
-- personal state without changing product behaviour (#807/#825).
alter table public.user_preferences
  drop column cbt_onboarding_completed,
  drop column act_onboarding_completed,
  drop column meditation_onboarding_completed,
  drop column meditation_info_completed,
  drop column gratitude_onboarding_completed,
  drop column habits_onboarding_completed,
  drop column mood_onboarding_completed,
  drop column journal_onboarding_completed,
  drop column sleep_onboarding_completed,
  drop column mindfulness_onboarding_completed,
  drop column grounding_onboarding_completed;

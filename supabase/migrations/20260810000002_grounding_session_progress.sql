-- Record how far a grounding session reached so an early finish can be shown
-- neutrally as "2 of 4" rather than being lost or presented as failure (#783).
alter table public.mindfulness_sessions_data
  add column steps_completed smallint,
  add column steps_total smallint,
  add constraint mindfulness_sessions_grounding_steps_check check (
    (steps_completed is null and steps_total is null)
    or (
      steps_completed is not null
      and steps_total is not null
      and steps_total between 1 and 20
      and steps_completed between 1 and steps_total
    )
  );
...
begin
  insert into public.mindfulness_sessions_data (
    ...
    completed_offset_minutes, steps_completed, steps_total)
  values (
    ...
    app.encrypt_text(new.feeling_after),
    new.cycles, new.duration_seconds, new.completed_offset_minutes,
    new.steps_completed, new.steps_total)
...
begin
  update public.mindfulness_sessions_data set
    ...
    feeling_after_enc        = app.encrypt_text(new.feeling_after),
    cycles                   = new.cycles,
    duration_seconds         = new.duration_seconds,
    completed_offset_minutes = new.completed_offset_minutes,
    steps_completed          = new.steps_completed,
    steps_total              = new.steps_total
   where id = old.id;
  return new;
end; $$;

-- Flat redeclaration from 20260805000000, extended only with grounding progress.

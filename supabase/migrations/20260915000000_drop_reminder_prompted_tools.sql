-- Drop reminder_prompted_tools (#2343, ADR-0008).
--
-- The column recorded one thing: which notification targets the post-save
-- reminder offer had already been shown for. It was the offer's own once-ever
-- bookkeeping - "the app asked you about mood" - never anything the person told
-- us, and never anything any surface read back to them.
--
-- #2342 removed the offer, so as of `06dabc9c` on `dev` nothing appends to the
-- list and nothing reads it. A column with no writer and no reader is stored
-- personal data kept for no reason, which is the privacy half of the ruling:
-- it goes, column and rows.
--
-- ⚠️ `reminder_consent` and `reminder_consent_updated_at` are deliberately NOT
-- touched. They record consent to the reminder *channel*, not to this prompt.
-- `reminder_consent` is a hard delivery gate - `send-web-reminders` skips any
-- user whose consent is falsy - and retiring the orphaned timestamp beside it is
-- a separate privacy decision, explicitly out of scope here (ADR-0008).
--
-- ⚠️ A shipped client that still sends this column is safe.
-- `updateUserPreferences` answers a PGRST204 "column not found" by dropping the
-- offending key and re-upserting (`src/features/settings/repository.ts`), so an
-- older build's write degrades into a write of everything else rather than an
-- error - which is why this drop does not have to wait on a release.

alter table public.user_preferences
  drop column if exists reminder_prompted_tools;

-- ═══════════════════════════════════════════════════════════════════════════════
-- export_user_data - redeclared from 20260914000000 minus reminder_prompted_tools
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- way in: 20260914000000 is the newest declaration as of 2026-09-12, and the
-- body below differs from it by exactly one line, the `reminder_prompted_tools`
-- entry in the preferences projection.
--
...
-- entry in `INTENTIONALLY_DROPPED` in
-- `test/export-user-data-monotonic.test.ts` - which is where the reason lives
-- for whoever meets that failure instead of this file.
...
-- PostgREST caches the schema, and this migration REMOVES a column older shipped
-- clients may still send. Without the reload their upserts meet a stale cache
-- that still lists the column and fail at the database instead - PGRST204 is the
-- error the client's retry knows how to recover from, and it is the reload that
-- produces it.

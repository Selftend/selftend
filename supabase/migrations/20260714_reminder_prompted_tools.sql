-- One-time contextual reminder prompt: track which notification targets the
-- prompt has already been shown for (same pattern as shown_button_tours).
-- Ordinary first-party preference state: exported via export_user_data() and
-- removed with the user_preferences row on account deletion.

alter table public.user_preferences
  add column if not exists reminder_prompted_tools text[] not null default '{}';

-- Per the convention in 20260704_export_user_data_flatten.sql: extend the flat
-- export definition with create or replace, adding the new column to the
-- preferences select. Body otherwise identical to 20260704.

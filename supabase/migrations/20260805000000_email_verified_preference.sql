-- Sign in without verification + persistent verify banner (#489).
--
-- With mailer_autoconfirm on, auth.users.email_confirmed_at is stamped at
-- signup and stops meaning "this mailbox was verified". The app's own flag
-- takes over: user_preferences.email_verified, set by the client after a
-- successful OTP code entry or an emailed-link round trip (both prove
-- mailbox access). Advisory only - it gates nothing but the banner.
--
...
-- declaration on dev, with ONLY that line added - verified by diff.
-- Versioned 20260805000000: full 14 digits, later than every existing

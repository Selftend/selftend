-- Explicit Art. 9(2)(a) consent, recorded on its own (#1766, spec #227 §3).
--
-- Until now the consent gate asked ONE question that carried three: an age
-- assertion, agreement to the terms and privacy policy, and consent to
-- Selftend processing the self-help entries the person chooses to save. The
-- third is special-category health data under GDPR Art. 9, and Art. 9(2)(a)
-- wants an EXPLICIT act for it - one that is separately worded, separately
-- given, and not a by-product of accepting a contract. Bundled into a terms
-- checkbox it is none of those things.
--
-- So the control splits in two in the app, and the record splits with it. This
-- column is the second half: it says the person performed that act and when.
-- privacy_policy_accepted_at / terms_accepted_at stay exactly what they were -
-- the contractual acceptance - and policy_version_accepted, written in the same
-- upsert, says which published wording the act was performed against.
--
-- NULLABLE with no default, and no backfill from privacy_policy_accepted_at.
-- That omission is the point rather than an oversight: every existing row DID
-- tick the old bundled checkbox, whose text did mention the processing, but the
-- whole reason this column exists is that a bundled tick is not the explicit,
-- separately-worded act Art. 9(2)(a) asks for. Backfilling would assert the
-- consent this ticket was raised to obtain. Existing accounts stay NULL and
-- perform the act once, when #1767's policy-version bump re-gates them through
-- the same consent slot - without being re-asked for age or country (§7).
--
-- No RLS changes, for the reason #1762 gave: user_preferences' policies are
-- row-level and scope by user_id, so they already cover every column the row
-- gains.
--
-- Version 20260906000000: strictly after 20260905000000_age_attestation.sql,
-- which is the newest declaration of export_user_data and the parent this one
-- was copied from (supabase/README.md "Migration versions").

alter table public.user_preferences
  add column if not exists health_data_consent_at timestamptz;

comment on column public.user_preferences.health_data_consent_at is
  'When the person gave explicit GDPR Art. 9(2)(a) consent to Selftend processing the self-help content they choose to enter (#1766, spec #227 §3). Its own affirmative act, separate from terms/privacy acceptance - never inferred from privacy_policy_accepted_at. NULL means the act has not been performed; withdrawal is deleting the account or contacting privacy, both of which remove the data rather than clearing this field.';

-- Redeclared IN FULL with health_data_consent_at added to the preferences
-- projection, per supabase/README.md "Modifying export_user_data". A consent
-- record the person cannot see in their own export is a consent record they
-- cannot check, and test/export-user-data-monotonic.test.ts holds the rest of
-- the projection to what the parent declaration already read.

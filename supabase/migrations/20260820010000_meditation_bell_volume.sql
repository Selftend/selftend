-- A meditation bell the user can turn down, or off (#1188).
--
-- All three bells fired at a hardcoded volume of 1, which measured as the
-- LOUDEST sound in the app: -14.1 LUFS-I against -17.2 for the guided breath
-- lane and -22.0 for the ambient beds, both of which HAVE a slider. The one
-- lane with no control was the one that needed it most.
--
-- Default 1, deliberately NOT quieter: #1130 owns absolute loudness and is
-- re-rendering these clips ~6 LU down. A default below 1 here would stack with
-- that re-render and over-attenuate the bell to inaudibility. This migration
-- ships the CONTROL; #1130 ships the level.
--
-- 0 means off. numeric not null default, no CHECK - the exact shape of
-- breath_volume and ambient_volume in 20260573, which this column joins.
--
-- export_user_data is redeclared with the one new column added to the main
-- preferences projection. Rebuilt from 20260820000000, the newest declaration
-- on dev, with ONLY that line added - verified by diff. Versioned
-- 20260820010000: full 14 digits, later than every existing version, sharing a
-- prefix with none (supabase/README.md "Migration versions").

alter table public.user_preferences
  add column bell_volume numeric not null default 1;

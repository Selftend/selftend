-- Put eleven columns of the user's own data into the GDPR export (#429).
--
-- Found by auditing the deployed `export_user_data` against the live schema
-- rather than against the previous declaration: parse every
-- `select … from public.<table>` in the function, union per table, diff against
-- `information_schema.columns`. #434's gate catches a redeclaration that DROPS a
-- column someone already exported; it is structurally unable to see a column
-- that no declaration ever carried, because there is nothing to compare against.
-- These are that second kind - none of them has ever been in the export.
--
--   journal_entries.occurred_at            the date the user CHOSE when
--                                          back-dating an entry. Not derived and
--                                          not internal - it is the whole reason
--                                          journal has an occurrence concept
--                                          separate from every other tool.
--   journal_entries.occurred_offset_minutes  the four #250 occurrence offsets.
--   mood_logs.logged_offset_minutes          The activity ones were repaired in
--   sleep_logs.logged_offset_minutes         #431; these simply never went in,
--   gratitude_entries.logged_offset_minutes  so no gate could have noticed.
--
--   mindfulness_sessions.feeling_after       session data the user entered.
--   mindfulness_sessions.cycles
--   mindfulness_sessions.duration_seconds
--
--   profiles.display_name                    the user's own profile. The export
--   profiles.avatar_url                      handed back an email and two
--   profiles.avatar_source                   timestamps and nothing they had set.
--   profiles.avatar_updated_at
--
-- Deliberately still withheld, so the next audit does not re-derive it:
-- `user_id` (the export is already scoped to the caller), `avatar_storage_path`
-- (internal storage plumbing, not a user-facing value - `avatar_url` is the
-- usable form), the push credentials `web_push_subscriptions.p256dh` / `auth`
-- and `device_push_tokens.expo_push_token`, and every `last_*_reminder_key`
-- (server bookkeeping). The ~34 unexported `user_preferences` columns are a
-- product question, not an oversight, and are left alone pending #429.
--
-- Rebuilt from 20260801000000, the newest declaration on dev, with ONLY the
-- lines above added - verified by diff against that file. `20260802000000` is a
-- later day so it sorts after it; a same-day timestamp would sort BEFORE the
-- short form and hand the export back to the older copy.

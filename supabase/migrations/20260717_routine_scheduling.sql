-- Routine scheduling schema (issue #103; design #96-#98 on map #95):
-- plaintext cadence + custom_days on routines_data, mirroring habits
-- (20260532_habits.sql) with the extra 'on-demand' value. A schedule is not
-- sensitive (unlike the encrypted name); plaintext keeps CHECK validation and
-- stays server-queryable for the schedule-aware reminder fan-out (#113).
--
-- Day numbering is JS getDay() convention (0=Sun..6=Sat), same as habits.

alter table public.routines_data
...
alter table public.routines_data
  drop constraint if exists routines_data_cadence_valid,
  add constraint routines_data_cadence_valid
    check (cadence in ('daily', 'weekdays', 'custom', 'on-demand'));

alter table public.routines_data
  drop constraint if exists routines_data_custom_days_valid,
  add constraint routines_data_custom_days_valid
    check (
      cadence <> 'custom'
      or (
...
-- ── routines view: append the new columns ─────────────────────────────────────
-- create or replace view can only APPEND columns, so cadence/custom_days go
-- last; consumers map by name, order is cosmetic. Triggers and grants survive.
...
-- ── INSTEAD OF triggers: carry the new columns ────────────────────────────────
...
begin
  ...
  values (
    ...
    coalesce(new.cadence, 'daily'), coalesce(new.custom_days, '{}'),
    coalesce(new.created_at, timezone('utc', now())), timezone('utc', now()))
  returning id, user_id, reminder_enabled, cadence, custom_days, created_at, updated_at
    into new.id, new.user_id, new.reminder_enabled, new.cadence, new.custom_days,
...
-- ── export_user_data: include the schedule in the routines export ─────────────

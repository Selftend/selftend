-- account_origin: which of the four doors an account was minted by (#2323).
--
-- `docs/marketing-plan.md` §3 defines arrivals as registered signups plus
-- web-CTA guests, excluding native cold-start guests. Until this column that
-- number was not computable at all: nothing in the schema distinguished a guest
-- who tapped "try it" on the website from a guest the app minted silently on
-- first launch. One nullable column makes the split readable
-- (`docs/measurement.md` §4, decided on #2306).
--
-- === What this is NOT ===============================================================
-- Not attribution, and the distinction is the whole reason this column is
-- allowed to exist. It records HOW an account came into being - a fact about
-- the app's own doors - never WHO sent the person. Acquisition source, channel,
-- referrer and campaign are refused capabilities (`docs/marketing-plan.md` §6,
-- #2307), and a surface label written onto a person's row is refused on
-- principle besides. `CONTEXT.md` §Accounts carries the vocabulary and the
-- "avoid" list. Nothing in the UI reads this column; its consumer is a human
-- running an aggregate query.
--
-- === Written on the device, not here ================================================
-- ☠️☠️ There is deliberately NO trigger and no default. The value cannot be
-- written at the mint: `signInWithOAuth` and `signInWithIdToken` cannot carry
-- custom user metadata, web OAuth loses any in-memory marker across the
-- full-page redirect, and this schema has no `auth.users` trigger and no
-- `handle_new_user()` to hang one on. Every mint-time scheme leaves at least one
-- of the four doors dark. So the client derives it at `AgeGate` - the earliest
-- gate, the one that covers every entry path - and stamps it with a conditional
-- UPDATE that matches only a null (`src/features/settings/repository.ts`).
--
-- ⚠️ No backfill, ever. The roughly 46 accounts that predate this column stay
-- null permanently: null means "minted before the fact was recorded", never
-- "refused" and never "other", and every available proxy for those accounts
-- would be a guess. A number spanning the boundary shows the unknown bucket
-- rather than folding it in (`docs/measurement.md` §4).
...
comment on column public.user_preferences.account_origin is
  'Which of the four doors this account was minted by (#2323, spec docs/measurement.md §4). Derived on-device and written ONCE, at the EARLIEST gate - today AgeGate, above ConsentGate in the shared gate slot. THE GATE ORDER IS LOAD-BEARING: the derivation reads platform and guest-ness off the live session, so a guest who has since converted reads as native + registered, and a gate running after a conversion would label a native_cold_start account native_signup. If you add a gate, leave this write where it is; the write-once null guard is what keeps a second write harmless rather than wrong. How the account came to be, never who sent the person - acquisition source, channel, referrer and attribution are refused (docs/marketing-plan.md §6). NULL means minted before this column existed: never backfilled, never "other".';

-- ═══════════════════════════════════════════════════════════════════════════════
-- export_user_data - redeclared from 20260910000000 plus account_origin
-- ═══════════════════════════════════════════════════════════════════════════════
-- ☠️ Redeclarations are last-writer-wins, so this copies the newest declaration
-- WHOLESALE rather than patching it - two in-flight migrations once silently
-- dropped each other's columns. Checked on the way in: 20260910000000 is the
-- newest declaration as of 2026-09-11 (20260911000000 and 20260913000000 only
-- mention `export_user_data` in comments), and the body below differs from it by
-- exactly one line, `account_origin` in the preferences projection.
--
-- The column is exported rather than withheld. It is not health data and not an
-- inferred trait - it is a fact about the account the person is holding - so
-- there is nothing here to keep back, and
-- `test/integration/export-user-data-completeness.integration.test.ts` requires
-- every live column to be one or the other.
...
-- PostgREST caches the schema, and this migration adds a column the client
-- writes. Without the reload the stamp fails as "column does not exist" until
-- the cache happens to refresh on its own - swallowed by the client, so it would
-- look like nothing at all while every account minted in the meantime stayed
-- null forever.

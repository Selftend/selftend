-- The meditation sit's ambient bed (#1742, decided in #1703 item 3).
--
-- Breathing has shipped nine looping beds since #1130 and a picker for them;
-- meditation had bells and, since #1188, a bell volume - but no bed. The bell
-- slider is the control #1139 said had to exist before a bed could sit beside
-- it, so this pair joins the breathing four from 20260573 with the same shape
-- (text id, numeric volume, no CHECK - a stored id is free text and the
-- repository resolves one the catalog lacks to 'none' on read, #1745).
--
-- Its OWN columns, deliberately not a reuse of ambient_sound_id: a person who
-- picked rain for breathing must not find it playing under a sit they never
-- asked it for. Default 'none' - a fresh account and every existing account
-- sits in silence exactly as before; the bed is a choice, never an expectation.
--
-- export_user_data is redeclared with the two columns added to the main
-- preferences projection. Rebuilt from 20260902000000, the newest declaration
-- on dev, with ONLY those two lines added - verified by diff. Versioned
-- 20260903000000: full 14 digits, later than every existing version, sharing a
-- prefix with none (supabase/README.md "Migration versions").

alter table public.user_preferences
  add column if not exists meditation_ambient_sound_id text not null default 'none',
  add column if not exists meditation_ambient_volume numeric not null default 0.5;

comment on column public.user_preferences.meditation_ambient_sound_id is
  'The looping bed under a meditation sit (#1742): an AMBIENT_SOUNDS id, ''none'' by default. Its own column so a bed chosen for breathing never plays under a sit uninvited.';
comment on column public.user_preferences.meditation_ambient_volume is
  'Volume of the sit''s ambient bed, 0..1 (#1742). The same shape as ambient_volume in 20260573.';

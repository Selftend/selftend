-- Consolidation: enrich mood_logs into a "Check-in" (four workbook boxes),
-- drop the noticing_logs table (its capture folds into mood), and remove
-- sleep/gratitude from self_care_logs (now owned by the shared Sleep + Gratitude tools).
...
alter table public.mood_logs
  ...
  add column if not exists behaviours text not null default '',
  add column if not exists bodily_sensations text not null default '';

drop table if exists public.noticing_logs;

alter table public.self_care_logs
  drop column if exists sleep_hours,
  drop column if exists sleep_quality,
  drop column if exists gratitude;

-- Re-declare export_user_data() to reflect the schema changes:
--   1. moodLogs select gains the four new columns.
--   2. noticingLogs block removed (table dropped above).
--   3. selfCareLogs select drops sleep_hours, sleep_quality, gratitude.

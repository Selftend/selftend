-- Haptic cues for the session tools (#1741, decided in #1703 item 2).
--
-- Every cue the meditation sit and the breathing session give is sound only:
-- three bells and one clip per breath phase, all through the audio path and
-- nothing else. A deaf or hard-of-hearing person, or anyone sitting with the
-- bell volume at 0, gets nothing from a bell or a phase boundary. This flag adds
-- the sound's counterpart - one tap per bell, one per phase - and it is native
-- only: src/lib/native-haptics.ts is a no-op on web (docs/accessibility.md).
--
-- boolean not null default false: off until asked for, on a fresh account and on
-- every existing one, the same promise meditation_bell_at_half makes
-- (20260828000000). Opt-in, off by default, a supplement and never required, as
-- #777's Motion decision already ruled for haptics ("Audio and haptics may
-- supplement [reduced-motion timing] but are never required"). Nothing in the
-- product surfaces it through a nudge or a prompt (product principle 12).
--
-- export_user_data is redeclared with the one new column added to the main
-- preferences projection. Rebuilt from 20260903000000, the newest declaration
-- on dev, with ONLY that line added - verified by diff. Versioned
-- 20260904000000: full 14 digits, later than every existing version, sharing a
-- prefix with none (supabase/README.md "Migration versions").

alter table public.user_preferences
  add column if not exists haptic_cues boolean not null default false;

comment on column public.user_preferences.haptic_cues is
  'Opt-in tap for each meditation bell and each breath phase boundary (#1741), the sound''s counterpart. Off by default; native only.';
